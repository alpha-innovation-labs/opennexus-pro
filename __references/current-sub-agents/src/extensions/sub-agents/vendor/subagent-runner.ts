...
import { isSubagentRunnerMode } from "../../../runtime/subagent-runner/isSubagentRunnerMode.js";
...
/**
 * Removes a temporary runner config file after loading it.
 *
 * @param configPath Temporary config file path.
 * @returns Void.
 */
function cleanupRunnerConfigFile(configPath: string): void {
	try {
...
/**
 * Reads runner config from a JSON file.
 *
 * @param configPath Config file path.
 * @returns Parsed runner config.
 */
function readSubagentRunnerConfigFile(configPath: string): SubagentRunConfig {
	const configJson = fs.readFileSync(configPath, "utf-8");
	const config = JSON.parse(configJson) as SubagentRunConfig;
	cleanupRunnerConfigFile(configPath);
	return config;
}

/**
 * Reads runner config JSON from stdin.
 *
 * @returns Parsed runner config.
 */
function readSubagentRunnerConfigFromStdin(): Promise<SubagentRunConfig> {
...
/**
 * Runs the detached subagent runner CLI mode.
 *
 * @param argv CLI arguments after the executable path.
 * @returns Promise that resolves when the runner has started.
 */
export async function runSubagentRunnerCli(argv: string[]): Promise<void> {
	const configPath = isSubagentRunnerMode(argv) ? argv[1] : argv[0];
	const config = configPath ? readSubagentRunnerConfigFile(configPath) : await readSubagentRunnerConfigFromStdin();
