...
import { rewriteSubagentPrompt } from "./subagent-prompt/rewriteSubagentPrompt.js";
...
export function buildPiArgs(input: BuildPiArgsInput): BuildPiArgsResult {
	...
	if (input.extensions !== undefined) {
		...
		for (const extPath of [...new Set([...toolExtensionPaths, ...input.extensions])]) {
			args.push("--extension", extPath);
		}
	} else {
		for (const extPath of [...new Set(toolExtensionPaths)]) {
			args.push("--extension", extPath);
		}
	}
	...
	if (input.systemPrompt !== undefined && input.systemPrompt !== null) {
		...
		const systemPrompt = rewriteSubagentPrompt(input.systemPrompt, {
