...
import { SUBAGENT_RUNNER_FLAG } from "../../../runtime/subagent-runner/isSubagentRunnerMode.js";
...
const runningBundledBinary = isBundledBinary(import.meta.url);
...
/**
 * Checks whether async execution support is available.
 *
 * @returns True when a compiled runner or jiti-based runner is available.
 */
export function isAsyncAvailable(): boolean {
	return runningBundledBinary || jitiCliPath !== undefined;
}
...
function spawnRunner(cfg: object, suffix: string, cwd: string): number | undefined {
	...
	const packageDir = process.env.PI_PACKAGE_DIR ?? (runningBundledBinary ? path.dirname(process.execPath) : undefined);
	if (runningBundledBinary) {
		const proc = spawn(process.execPath, [SUBAGENT_RUNNER_FLAG, cfgPath], {
			cwd,
			detached: true,
			stdio: "ignore",
			windowsHide: true,
