...
/**
 * Returns a workflow Engineer mode prompt when the mode is supported.
 *
 * @param mode Workflow Engineer mode name.
 * @returns Mode-specific prompt text or undefined.
 */
export function getWorkflowEngineerModePrompt(mode: string | undefined): string | undefined {
  if (!mode) return undefined;
  return ENGINEER_MODE_PROMPTS[mode.trim().toLowerCase()];
}

/**
 * Lists the workflow Engineer modes supported by prompt injection.
 *
 * @returns Supported workflow Engineer mode names.
 */
export function getWorkflowEngineerModes(): string[] {
