...
import { createAgentActivityTracker } from "./ui/createAgentActivityTracker.js";
...
- Use isolation: "worktree" to run the agent in an isolated git worktree (safe parallel file modifications).`,
    ...
    execute: async (toolCallId, params, signal, onUpdate, ctx) => {
      ...
      if (runInBackground) {
        const { state: bgState, callbacks: bgCallbacks } = createAgentActivityTracker(effectiveMaxTurns);
      ...
      const streamUpdate = () => {
        const details: AgentDetails = {
          ...
          status: "running",
          activity: describeActivity(fgState.activeTools, fgState.thinkingText, fgState.responseText),
      ...
      const { state: fgState, callbacks: fgCallbacks } = createAgentActivityTracker(effectiveMaxTurns, streamUpdate);
