 ...
 */
export function buildReadFailureMessage(action: string, query: string, error: unknown): string {
  const message = error instanceof Error ? error.message : String(error);
  return message.includes(query) ? message : `Could not ${action} \"${query}\". ${message}`;
}
