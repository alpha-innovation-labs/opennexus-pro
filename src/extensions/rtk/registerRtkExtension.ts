...
export function registerRtkExtension(pi: ExtensionAPI): void {
  ...
  pi.on("session_start", async (_event, ctx) => {
    const available = await hasRtkBinary(pi, ctx.cwd, ctx.signal);
    if (!available) {
      clearRtkRuntimeForCwd(ctx.cwd);
      console.warn("[rtk] rtk binary not found in PATH — extension falls back to base tools");
      return;
    }

    setRtkRuntimeForCwd(ctx.cwd, createRtkRuntime(pi));
  });
  ...
  pi.on("tool_call", async (event, ctx) => {
    ...
    try {
      ...
      const result = await runtime.exec("rewrite", [command], { cwd: ctx.cwd, signal: ctx.signal });
