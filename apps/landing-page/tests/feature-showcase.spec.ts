...
test.describe("Nexus feature showcase", () => {
  test("mounts one showcase terminal player without duplicate ids", async ({ page }) => {
    await page.goto("/");
    ...
    await expect(page.locator(".showcase-terminal-player .ap-player")).toBeVisible({ timeout: 10_000 });
  ...
  test("settles the terminal before the first showcase item is passed", async ({ page }) => {
    await page.setViewportSize({ width: 1920, height: 1080 });
    await page.goto("/");
    const featureTop = await page.locator("#features").evaluate((element) => element.getBoundingClientRect().top + window.scrollY);
    await page.evaluate((scrollTop) => window.scrollTo(0, scrollTop), featureTop);
    await page.waitForTimeout(250);

    const introLayout = await page.evaluate(() => {
      const terminal = document.querySelector<HTMLElement>(".showcase-terminal-figure")?.getBoundingClientRect();
      const copy = document.querySelector<HTMLElement>(".feature-story-copy");
      if (!terminal || !copy) return null;
      return {
        terminalCenterDelta: Math.abs((terminal.left + terminal.width / 2) - window.innerWidth / 2),
        copyOpacity: Number(window.getComputedStyle(copy).opacity),
      };
    });

    await page.evaluate((scrollTop) => window.scrollTo(0, scrollTop + 160), featureTop);
    await page.waitForTimeout(250);

    const settledLayout = await page.evaluate(() => {
      const terminal = document.querySelector<HTMLElement>(".showcase-terminal-figure")?.getBoundingClientRect();
      const copy = document.querySelector<HTMLElement>(".feature-story-copy")?.getBoundingClientRect();
      const firstTitle = document.querySelector<HTMLElement>("#code-lsp [data-showcase-story-title]")?.getBoundingClientRect();
  ...
  test("shows attached showcase navigation only after the terminal settles", async ({ page }) => {
    ...
    await expect(page.locator('[data-showcase-child-link="code-lsp"]')).toBeVisible();
    await expect(page.locator('[data-showcase-child-link="code-fast-files"]')).toBeVisible();
    await expect(page.locator('[data-showcase-child-link="code-web-mcp"]')).toBeVisible();
    ...
    const shellWidths = await page.evaluate(() => {
      const nav = document.querySelector<HTMLElement>(".nav-inner")?.getBoundingClientRect();
      const featureNav = document.querySelector<HTMLElement>(".feature-nav-inner")?.getBoundingClientRect();
  ...
  test("keeps the desktop showcase compact with copy beside the terminal", async ({ page }) => {
    ...
    const layout = await page.evaluate(() => {
      ...
      const terminal = document.querySelector<HTMLElement>(".showcase-terminal-figure");
      const copy = document.querySelector<HTMLElement>(".feature-story-copy");
  ...
  test("keeps the original cast while feature sections scroll", async ({ page }) => {
    await page.goto("/");
    await page.locator("#features").scrollIntoViewIfNeeded();
    await expect(page.locator(".showcase-terminal-player")).toHaveAttribute("data-cast-src", "/recordings/demo.cast");

    await page.locator("#custom-promptline").evaluate((element) => element.scrollIntoView({ block: "center" }));
    await page.evaluate(() => window.scrollBy(0, 160));
    await expect(page.locator(".showcase-terminal-player")).toHaveAttribute("data-cast-src", "/recordings/demo.cast");
    await expect(page.locator("#custom-promptline")).toHaveClass(/opacity-100/);
  });

  test("uses a sticky terminal story beside the showcase copy", async ({ page }) => {
    await page.goto("/");
    await page.locator("#features").scrollIntoViewIfNeeded();

    const storyStyle = await page.locator(".feature-story-grid").evaluate((element) => {
      const style = window.getComputedStyle(element);
      return { display: style.display, columns: style.gridTemplateColumns };
    });
    const terminalPosition = await page.locator(".showcase-terminal-figure").evaluate((element) => window.getComputedStyle(element).position);
  ...
  test("stacks the terminal story on small screens without static media cards", async ({ page }) => {
    await page.setViewportSize({ width: 390, height: 844 });
    await page.goto("/");

    const layout = await page.locator(".feature-story-grid").evaluate((element) => {
      const style = window.getComputedStyle(element);
      return { columns: style.gridTemplateColumns, staticCards: document.querySelectorAll("#features .feature-video-card").length };
    });
