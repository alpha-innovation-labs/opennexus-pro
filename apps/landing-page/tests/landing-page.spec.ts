...
test.describe("Nexus landing page", () => {
  ...
  test("uses square corners across page chrome", async ({ page }) => {
    ...
    const curvedElements = await page.evaluate(() => {
      ...
      const cornerStyles = ["top-left", "top-right", "bottom-right", "bottom-left"].map((corner) => `border-${corner}-${cornerStyleSuffix}`);

      return [...document.querySelectorAll("body *")].flatMap((element) => {
        const style = window.getComputedStyle(element);
        const cornerValues = cornerStyles.map((cornerStyle) => style.getPropertyValue(cornerStyle));
        const hasCurvedCorner = cornerValues.some((cornerValue) => Number.parseFloat(cornerValue) > 0);
        if (!hasCurvedCorner) return [];

        return [`${element.tagName.toLowerCase()} ${element.getAttribute("class") ?? ""}: ${cornerValues.join(",")}`];
      });
    });
