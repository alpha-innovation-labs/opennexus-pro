...
export function InstallBox() {
	...
	const activeInstall = useMemo(
		() => getActiveInstallOption(installOptions, activeId),
		[activeId],
	);
		...
		<div className="install-box w-[350px] max-w-full overflow-hidden border border-border/60 bg-card text-left text-foreground">
			<div className="flex flex-col">
				<div
					...
					className="install-tabs flex flex-wrap border-b border-border/60 bg-muted/40"
					tabIndex={0}
				>
				...
				<div
					className="install-command-row flex items-start gap-3 bg-background px-4 py-3"
					data-install-command={activeInstall?.command}
				>
					<span className="pt-0.5 text-emerald-500" aria-hidden="true">
						$
					</span>
