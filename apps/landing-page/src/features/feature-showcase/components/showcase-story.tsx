...
export function ShowcaseStory(props: ShowcaseStoryProps) {
  ...
  const items = useMemo(() => flattenShowcaseTerminalItems(props.groups), [props.groups]);
  ...
  useEffect(() => {
    ...
    function updateStoryState(): void {
      ...
      const progress = getShowcaseStoryProgress(root.getBoundingClientRect(), window.innerHeight);
      const sections = Array.from(root.querySelectorAll<HTMLElement>("[data-showcase-story-example]"));
      const markers = createShowcaseStoryMarkers(sections);
      const nextActiveId = resolveActiveShowcaseStoryId(markers, window.innerHeight * 0.32, items[0].example.id);
      ...
      if (terminalHeight > 0) {
        root.style.setProperty("--showcase-terminal-sticky-top", `${getShowcaseTerminalStickyTop(window.innerHeight, terminalHeight, navHeight)}px`);
      }
    ...
    updateStoryState();
    window.addEventListener("scroll", scheduleStoryStateUpdate, { passive: true });
    window.addEventListener("resize", scheduleStoryStateUpdate, { passive: true });
  ...
  return (
    <div ref={rootRef} className="feature-story-grid mx-auto grid w-full max-w-[1728px] grid-cols-[minmax(0,1.4fr)_clamp(396px,calc(100%-var(--page-shell-gap)-var(--home-demo-column-floor)),648px)] gap-[var(--page-shell-gap)] px-[36px] py-12 [--home-demo-column-floor:720px] [--page-shell-gap:clamp(45px,4vw,90px)] [--showcase-story-progress:0] [--showcase-terminal-sticky-top:calc(50vh-12rem)] max-[1023px]:grid-cols-1 max-[1023px]:gap-0 max-[1023px]:px-8 max-[860px]:px-4 max-[860px]:py-8">
      <div className="showcase-terminal-rail z-20 flex min-h-[calc(100vh-8rem)] items-start max-[860px]:min-h-0">
        <figure
          ...
          style={{ transform: "translate3d(calc((1 - var(--showcase-story-progress)) * 16vw), 0, 0) scale(calc(1 + (1 - var(--showcase-story-progress)) * 0.08))", transformOrigin: "center center" }}
        >
          ...
          {activeItem ? <ShowcaseTerminalPlayer key={activeItem.castSrc} castSrc={activeItem.castSrc} title={activeItem.example.title} /> : null}
        </figure>
      </div>

      <div className="feature-story-copy grid gap-14 pt-[clamp(10rem,18vh,14rem)] transition-opacity duration-300 max-[860px]:pt-0" style={{ opacity: "var(--showcase-story-progress)" }}>
        {props.groups.map((group) => (
          <section className="showcase-group scroll-mt-32" id={group.id} data-showcase-major={group.id} key={group.id}>
            <div className="showcase-group-heading grid gap-4 pb-8">
              <p className="eyebrow m-0 font-mono text-xs font-semibold uppercase leading-tight tracking-[0.12em] text-neutral-500 dark:text-neutral-400">{group.eyebrow}</p>
              <h2 className="m-0 max-w-2xl text-[clamp(2rem,3.3vw,3.25rem)] font-semibold leading-tight tracking-[-0.045em] text-neutral-950 text-balance dark:text-neutral-100">{group.title}</h2>
              <p className="m-0 max-w-2xl text-lg leading-relaxed text-neutral-600 dark:text-neutral-400">{group.body}</p>
            </div>
            <div className="grid gap-10">
              {group.examples.map((example) => {
                const isActive = example.id === activeId;
                return (
                  <article className={cn("showcase-example scroll-mt-32 border-l-2 py-4 pl-6 transition duration-200", isActive ? "border-cyan-400 opacity-100" : "border-neutral-200 opacity-45 dark:border-white/10")} id={example.id} data-showcase-child={example.id} data-showcase-story-example="true" key={example.id}>
                    <p className="eyebrow m-0 pb-3 font-mono text-xs font-semibold uppercase leading-tight tracking-[0.12em] text-neutral-500 dark:text-neutral-400">{example.eyebrow}</p>
                    <h3 className="m-0 text-[clamp(1.35rem,1.75vw,1.8rem)] font-semibold leading-tight tracking-[-0.035em] text-neutral-950 text-balance dark:text-neutral-100" data-showcase-story-title="true">{example.title}</h3>
                    <p className="m-0 pt-4 text-base leading-relaxed text-neutral-600 dark:text-neutral-400">{example.body}</p>
                    <ul className="m-0 grid list-none gap-2 pt-4 p-0">
                      {example.bullets.map((bullet) => <li className="font-mono text-xs font-semibold leading-relaxed text-neutral-800 dark:text-neutral-200" key={bullet}>{bullet}</li>)}
                    </ul>
                  </article>
                );
              })}
