...
export function ShowcaseVideo(props: ShowcaseVideoProps) {
  return (
    <aside className="feature-video-card bg-neutral-950 p-5 text-neutral-100 max-[860px]:hidden" aria-label={`${props.title} visual summary`}>
      ...
      <div className="feature-video-preview grid min-h-[20rem] place-items-center border border-white/10 bg-[radial-gradient(circle_at_30%_20%,rgba(129,140,248,0.26),transparent_34%),linear-gradient(135deg,#020617,#111827)] p-8" aria-hidden="true">
        <div className="feature-preview-window grid w-full max-w-md gap-4 border border-white/10 bg-black/35 p-6 shadow-2xl">
          ...
          <div className="feature-preview-rows grid gap-2 font-mono text-xs font-medium">
            {props.bullets.slice(0, 3).map((bullet) => <span className="border border-white/10 bg-white/5 px-3 py-2 text-neutral-200" key={bullet}>{bullet}</span>)}
          </div>
        </div>
      </div>
    </aside>
