...
export function ShowcaseNav(props: ShowcaseNavProps) {
	const [activeGroupId, setActiveGroupId] = useState(
		getFirstShowcaseGroupId(props.groups),
	);
	const [activeExampleId, setActiveExampleId] = useState(
		getFirstShowcaseExampleId(props.groups),
	);
	...
	useEffect(() => {
		...
		updateNavVisibility();
		window.addEventListener("scroll", scheduleNavVisibilityUpdate, {
			passive: true,
		});
		window.addEventListener("resize", scheduleNavVisibilityUpdate, {
			passive: true,
		});
	...
	return (
		<nav
			className={cn(
				"feature-sticky-nav sticky top-16 z-40 border-b border-neutral-200/70 bg-neutral-50/92 backdrop-blur-lg transition duration-200 dark:border-white/10 dark:bg-black/90 max-[860px]:static max-[860px]:overflow-x-auto",
				!isSettled && "pointer-events-none opacity-0",
			)}
			aria-label="Feature showcase navigation"
			...
			<div className="feature-nav-inner mx-auto grid max-w-[1728px] gap-2 px-[36px] py-3 max-[860px]:min-w-max max-[860px]:px-8 max-[560px]:px-4">
				<div className="feature-major-nav flex flex-wrap items-center gap-2 font-mono text-xs font-semibold uppercase tracking-[0.12em]">
					{props.groups.map((group) => (
						<Link
							className={cn(
								"px-3 py-2 text-neutral-600 transition hover:bg-neutral-100 hover:text-neutral-950 dark:text-neutral-400 dark:hover:bg-white/10 dark:hover:text-neutral-100",
								activeGroupId === group.id &&
									"bg-neutral-950 text-white dark:bg-white dark:text-black",
							)}
							href={`#${group.id}`}
							data-showcase-major-link={group.id}
							key={group.id}
				...
				<div className="feature-child-nav flex flex-wrap items-center gap-2 font-mono text-xs font-semibold">
					{props.groups.flatMap((group) =>
						group.examples.map((example) => (
							<Link
								className={cn(
									"px-3 py-2 text-neutral-500 transition hover:bg-neutral-100 hover:text-neutral-950 dark:text-neutral-500 dark:hover:bg-white/10 dark:hover:text-neutral-100",
									activeExampleId === example.id &&
										"bg-neutral-950 text-white dark:bg-white dark:text-black",
								)}
								href={`#${example.id}`}
								data-showcase-child-link={example.id}
								data-showcase-parent={group.id}
