...
export function ShowcaseExample(props: ShowcaseExampleProps) {
	return (
		<article
			className="showcase-example grid scroll-mt-32 grid-cols-[minmax(0,0.88fr)_minmax(18rem,1.12fr)] overflow-hidden border border-neutral-200/70 bg-neutral-50/80 dark:border-white/10 dark:bg-black/40 max-[860px]:grid-cols-1"
			id={props.example.id}
			data-showcase-child={props.example.id}
		>
			<div className="feature-copy-card grid content-start gap-4 p-[clamp(1.25rem,3vw,2.5rem)]">
				...
				<ul className="m-0 grid list-none gap-2 p-0">
					{props.example.bullets.map((bullet) => (
						<li
