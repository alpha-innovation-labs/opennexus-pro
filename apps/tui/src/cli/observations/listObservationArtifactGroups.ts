 ...
 */
export async function listObservationArtifactGroups(
	observationsDir: string,
): Promise<ObservationArtifactGroup[]> {
	...
	for (const fileName of fileNames) {
		...
		if (!parsed) continue;
		const group = groups.get(parsed.conversationId) ?? {
			conversationId: parsed.conversationId,
			sessionId: getObservationSessionId(parsed.conversationId),
		};
 ...
 */
function parseObservationArtifactFileName(fileName: string):
	| {
			conversationId: string;
			kind: "messages" | "state" | "legacy-state" | "markdown";
	  }
	| undefined {
	const legacyMatch =
		/^(.*)\.(messages\.json|state\.json|observations\.md)$/.exec(fileName);
	if (legacyMatch) {
