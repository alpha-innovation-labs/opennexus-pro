...
import { matchesObservationRecreateSessionTarget } from "./matchesObservationRecreateSessionTarget";
...
export function selectObservationRecreateSessions(
	...
	const exactMatches = sessions.filter((session) =>
		matchesObservationRecreateSessionTarget(session, target, true),
	);
	const matches =
		exactMatches.length > 0
			? exactMatches
			: sessions.filter((session) =>
					matchesObservationRecreateSessionTarget(session, target, false),
				);
	if (matches.length === 0)
		return { error: `No session found matching '${target}'` };
	if (matches.length > 1)
		return {
			error: `Multiple sessions match '${target}'. Use a longer session id.`,
		};
