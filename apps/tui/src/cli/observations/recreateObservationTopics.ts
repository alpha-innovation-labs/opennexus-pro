...
export async function recreateObservationTopics(
	...
	const output = await runObservationSummarizer(
		RECREATE_EXTENSION_API,
		{ cwd },
		await buildObservationRecreationPrompt(messages),
	);
	const topics = parseRecreatedObservationTopics(output);
	if (topics.length === 0) {
		console.error(
			`[observations recreate] LLM returned 0 topics for ${messages.length} messages.`,
		);
		console.error(
			`[observations recreate] Raw LLM output: ${JSON.stringify(output)}`,
		);
	}
	return topics;
}
