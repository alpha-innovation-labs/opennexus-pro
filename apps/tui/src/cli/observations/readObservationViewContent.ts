 ...
 */
export async function readObservationViewContent(
	target: string,
	...
	if (groups.length === 0)
		return { error: `No observations found matching '${target}'` };
	if (groups.length > 1)
		return {
			error: `Multiple observations match '${target}'. Use a longer session id.`,
		};
	const group = groups[0];
	const statePath = group.statePath ?? group.legacyStatePath;
	if (!statePath)
		return { error: `No observation state found for '${target}'` };
