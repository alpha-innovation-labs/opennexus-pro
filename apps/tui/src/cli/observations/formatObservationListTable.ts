 ...
 */
export function formatObservationListTable(rows: readonly ObservationListJsonRow[]): string {
...
function formatFiles(row: ObservationListJsonRow): string {
  return [row.hasMessages ? "messages" : "missing-messages", row.hasState ? "state" : "missing-state", row.hasMarkdown ? "markdown" : "missing-markdown"].join(",");
}
