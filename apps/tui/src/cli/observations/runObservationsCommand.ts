 ...
 */
export async function runObservationsCommand(
...
async function runList(
	...
	console.log(
		json ? formatObservationListJson(rows) : formatObservationListTable(rows),
	);
	return 0;
}
...
async function runDelete(target: string | undefined): Promise<number> {
	...
	console.log(
		`Deleted ${deletedCount} observation group${deletedCount === 1 ? "" : "s"}`,
	);
	return 0;
}
...
async function runRecreate(
	...
	await Promise.all(
		selected.sessions.map((session) =>
			recreateObservationArtifactsForSession(session),
		),
	);
	console.log(
		`Recreated ${selected.sessions.length} observation group${selected.sessions.length === 1 ? "" : "s"}`,
	);
	return 0;
}
