 ...
 */
export function parseObservationsCommand(argv: readonly string[]): { request: ObservationCliRequest } | { error: string } {
  const action = argv[1];
  ...
  if (action !== "list" && json) return { error: "--json is only supported for observations list." };
