 ...
 */
export function calculateSessionTableColumnWidths(
	terminalWidth?: number,
): SessionTableColumnWidths {
	const safeTerminalWidth = Math.max(
		MIN_TABLE_WIDTH,
		Math.floor(terminalWidth ?? DEFAULT_TERMINAL_WIDTH),
	);
	const fullIdLayoutTitleWidth =
		safeTerminalWidth -
		DATE_COLUMN_WIDTH -
