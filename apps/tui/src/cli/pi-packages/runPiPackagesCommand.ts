 ...
 */
export async function runPiPackagesCommand(argv: readonly string[]): Promise<number | undefined> {
 ...
 * @returns Exit code.
 */
async function runToggleCommand(action: string, options: { source?: string }): Promise<number> {
  ...
  try {
    ...
    console.log(`${action === "enable" ? "Enabled" : "Disabled"} ${source}. Restart Nexus to apply.`);
    return 0;
  } catch (error) {
