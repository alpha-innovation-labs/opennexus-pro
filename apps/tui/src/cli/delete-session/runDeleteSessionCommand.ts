...
export async function runDeleteSessionCommand(
	...
	const result = await resolveDeleteSessionTarget(
		sessionReference,
		cwd,
		sessionDir,
	);
	...
	if (result.type === "ambiguous") {
		console.error(
			formatAmbiguousDeleteSessionMessage(
	...
	const deletedObservationCount =
		await deleteObservationArtifactsForSessionPath(result.session.path);
	await deleteSessionFile(result.session.path);
	console.log(`Deleted session ${result.session.id}`);
	if (deletedObservationCount > 0)
		console.log(
			`Deleted ${deletedObservationCount} observation group${deletedObservationCount === 1 ? "" : "s"}`,
		);
	return 0;
}
