...
export async function extractVendorPackage(entry) {
  ...
  try {
    const packed = (await runCommand("npm", ["pack", entry.source, "--silent"], { cwd: workspace })).trim().split("\n").at(-1);
    if (!packed) throw new Error(`npm pack returned no tarball for ${entry.source}`);
    const extractDir = join(workspace, "package");
    await mkdir(extractDir, { recursive: true });
    await runCommand("tar", ["-xzf", basename(packed), "-C", extractDir, "--strip-components=1"], { cwd: workspace });
