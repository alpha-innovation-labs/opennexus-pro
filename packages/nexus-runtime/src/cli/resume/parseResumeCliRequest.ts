 ...
 */
export function parseResumeCliRequest(argv: readonly string[]): ResumeCliRequest {
  for (let index = 0; index < argv.length; index += 1) {
    const arg = argv[index];

    if (isResumeSelectorFlag(arg)) {
      const next = argv[index + 1];
      if (typeof next === "string" && !next.startsWith("-") && looksLikeSessionReference(next)) {
        return { mode: "direct", flagIndex: index, target: next, consumesNextArg: true };
      }
    ...
    if (arg.startsWith("--resume=")) {
      ...
      if (looksLikeSessionReference(target)) {
        return { mode: "direct", flagIndex: index, target, consumesNextArg: false };
      }
    }
  }
