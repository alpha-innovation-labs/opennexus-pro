...
export async function applyNexusConfigPatch(): Promise<void> {
	...
	const patchedSettingsManager =
		SettingsManager as unknown as NexusSettingsManagerClass;
	...
	patchedSettingsManager.fromStorage = function fromStorageWithNexusDefaults(
		storage: unknown,
	) {
		const manager = originalFromStorage.call(
			this,
			storage,
		) as unknown as NexusSettingsManagerInstance;
		manager.globalSettings = mergeSettings(appDefaults, manager.globalSettings);
		manager.settings = mergeSettings(
		...
		if (nexusExt && typeof nexusExt === "object" && !Array.isArray(nexusExt)) {
			...
			if (piPackages && typeof piPackages === "object") {
				...
				const existingPackages =
					((manager.globalSettings as Record<string, unknown>)
						.packages as string[]) ?? [];
				const merged = [...existingPackages];
				for (const [src, enabled] of Object.entries(
					piPackages as Record<string, unknown>,
				)) {
		...
		manager.getGlobalSettings =
			function getGlobalSettingsWithNexusConversion() {
