...
export interface WorkflowCardInput {
  ...
  /** Transcript summary only; the expanded card retains the tree and logs. */
  compact?: boolean;
}
...
export function layoutWorkflowCard(input: WorkflowCardInput): WorkflowCardLine[] {
  ...
  if (input.compact) {
    // Put lifecycle first so it survives narrow terminals; counts remain actual.
    return [clampLine([{ text: `${input.task.status} · ${head.name} · ${head.stats}`, color: "toolTitle" }], width)];
  }
