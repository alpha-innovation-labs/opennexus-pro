...
const VALID_TIME_RANGES: WebSearchTimeRange[] = [
	"day",
	"week",
	"month",
	"year",
...
export function registerWebSearchTool(
	...
	pi.registerTool(
		defineTool({
			...
			label: "Web Search",
			description:
				"Search the web using a SearXNG instance and return up to 10 results as title/url/snippet tuples. Supports optional filters: max_results, region, language, time_range, categories, safe_search.",
			promptSnippet:
				"Use web_search to search the web for information. Specify query and optional filters like max_results, region, language, time_range, categories, or safe_search.",
			parameters: Type.Object({
				...
				region: Type.Optional(
					Type.String({
						description:
							"Region code for search localization (e.g., 'us-en', 'de-de').",
					}),
				),
				language: Type.Optional(
					Type.String({
						description: "Language code for results (e.g., 'en', 'de', 'zh').",
					}),
				),
				time_range: Type.Optional(
					Type.Union(
						VALID_TIME_RANGES.map((t) => Type.Literal(t)),
				...
				categories: Type.Optional(
					Type.Array(
						Type.Union(
							VALID_CATEGORIES.map((c) => Type.Literal(c)),
							{
								description:
									"Search categories: general, news, videos, images, music, files, it, science, map.",
							},
						),
					),
				),
