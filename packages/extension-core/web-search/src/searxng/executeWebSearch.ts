 ...
 */
function buildSearXNGUrl(baseUrl: string, params: WebSearchParams): string {
	...
	if (params.time_range)
		url.searchParams.set("time_range", params.time_range as string);
	...
	try {
		...
		if (!response.ok) {
			...
			if (response.status === 403) {
				return {
					...
					query: params.query,
					error:
						"SearXNG JSON API is not enabled. Ensure 'json' is listed under 'formats:' in settings.yml and restart the container.",
				};
			}
			return {
				results: [],
		...
		try {
			...
			return {
				...
				query: params.query,
				error:
					"SearXNG returned malformed JSON after stripping control characters.",
			};
		}
