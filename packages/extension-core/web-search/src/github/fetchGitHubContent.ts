...
export async function fetchGitHubContent(
	...
	const ref =
		info.ref || (await getDefaultBranch(info.owner, info.repo, signal));
	const title = info.path
		? `${info.owner}/${info.repo} - ${info.path}`
		: `${info.owner}/${info.repo}`;
	if (info.type === "blob" && info.path) {
		const file = await fetchRawGitHubFile(
			info.owner,
			info.repo,
			ref,
			info.path,
	...
	const [tree, readme] = await Promise.all([
		...
		info.type === "root"
			? fetchGitHubReadme(info.owner, info.repo, ref, signal)
