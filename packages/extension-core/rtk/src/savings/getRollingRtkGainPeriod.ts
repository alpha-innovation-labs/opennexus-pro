 ...
 */
export function getRollingRtkGainPeriod(periods: RtkGainPeriod[] | undefined, days: number): RtkGainPeriod | undefined {
  const dated = (periods ?? [])
    .map((period) => ({ date: getRtkGainDailyDate(period), period }))
    .filter((entry): entry is { date: Date; period: RtkGainPeriod } => entry.date !== undefined)
  ...
  const aggregated = aggregateRtkGainPeriods(included.map((entry) => entry.period));
