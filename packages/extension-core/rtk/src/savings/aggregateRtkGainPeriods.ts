 ...
 */
export function aggregateRtkGainPeriods(periods: RtkGainPeriod[]): RtkGainPeriod | undefined {
  if (periods.length === 0) return undefined;
  const total = periods.reduce<RtkGainPeriod>((accumulator, period) => ({
    avg_time_ms: 0,
  ...
  }), { avg_time_ms: 0, commands: 0, input_tokens: 0, output_tokens: 0, saved_tokens: 0, savings_pct: 0, total_time_ms: 0 });
  total.avg_time_ms = total.commands > 0 ? total.total_time_ms / total.commands : 0;
  total.savings_pct = total.input_tokens > 0 ? (total.saved_tokens / total.input_tokens) * 100 : 0;
  return total;
}
