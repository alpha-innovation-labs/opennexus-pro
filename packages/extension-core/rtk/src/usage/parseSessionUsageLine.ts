...
export function parseSessionUsageLine(line: string): SessionUsageLine | null {
	...
	const cacheWrite =
		Number((usage as { cacheWrite?: unknown }).cacheWrite) || 0;
	const total =
		Number((usage as { totalTokens?: unknown }).totalTokens) ||
		input + output + cacheRead + cacheWrite;
	const timestampValue =
		(record as { timestamp?: unknown }).timestamp ??
	...
	return {
		...
		timestamp,
		usage: { cacheRead, cacheWrite, input, output, total },
	};
}
