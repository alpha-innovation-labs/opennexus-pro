 ...
 */
export function aggregateTokenUsagePeriods(
	key: string,
	...
	for (const period of periods) {
		...
		for (const [model, tokens] of Object.entries(period.modelTokens))
			aggregate.modelTokens[model] =
				(aggregate.modelTokens[model] ?? 0) + tokens;
	}
	return aggregate;
}
