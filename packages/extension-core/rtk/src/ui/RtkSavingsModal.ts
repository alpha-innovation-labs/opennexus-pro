...
export class RtkSavingsModal extends SharedModal {
	...
	constructor(
		...
		super({
			...
			panes: [
				{
					id: "savings",
					lines: createRtkSavingsLines(report, theme, "daily"),
					size: 1,
				},
			],
			theme,
	...
	override render(width: number): string[] {
		this.headerLines = [
			createRtkSavingsHeaderLine(
				this.theme,
				this.selectedPeriod,
				getRtkSavingsModalInnerWidth(width),
			),
		];
	...
	private selectPeriod(direction: 1 | -1): void {
		this.selectedPeriod = getNextRtkSavingsPeriod(
			this.selectedPeriod,
			direction,
		);
	...
	private openModelPanel(): void {
		...
		this.modelSelectionIndex = Math.max(
			0,
			models.findIndex(
				(model) => model.id === this.getSelectedPricingModelId(),
			),
		);
	...
	private handleModelPanelInput(data: string): boolean {
		...
		if (data === "j" || matchesKey(data, Key.down))
			return this.moveModelSelection(1);
	...
	private moveModelSelection(direction: 1 | -1): true {
		...
		this.modelSelectionIndex = Math.max(
			0,
			Math.min(models.length - 1, this.modelSelectionIndex + direction),
		);
		this.refreshModelPanelLines();
		return true;
	}
	...
	private refreshModelPanelLines(): void {
		this.footerLines = [
			this.theme.fg(
				"dim",
				`Filter: ${this.modelFilterQuery || "type to filter"} · Enter select · Esc back`,
			),
		];
		this.panes = [
			{ id: "models", lines: this.createModelPanelLines(), size: 1 },
		];
		this.onRenderNeeded();
	}
	...
	private createModelPanelLines(): string[] {
		...
		if (models.length === 0)
			return [
				this.theme.fg("accent", "Pricing model"),
				this.theme.fg("muted", "No models match the filter."),
			];
		return [
			this.theme.fg("accent", "Pricing model"),
	 ...
	 */
	private getVisibleFilteredModels(): Array<{
		index: number;
		...
		const start = Math.max(
			0,
			Math.min(
				this.modelSelectionIndex - Math.floor(maxRows / 2),
				models.length - maxRows,
			),
		);
		return models
			.slice(start, start + maxRows)
			.map((model, offset) => ({ index: start + offset, model }));
	}
	...
	private getAvailableModels(): OpenRouterModelOption[] {
		return "availableModels" in this.report
			? (this.report.availableModels ?? [])
			: [];
	}
	...
	private getSelectedPricingModelId(): string | undefined {
		return "pricingModelId" in this.report
			? this.report.pricingModelId
			: undefined;
	}
}
