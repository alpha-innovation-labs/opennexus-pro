 ...
 */
export function calculateTopLevelMenuWidth(
	...
	const widestRow = items.reduce((widest, item) => {
		...
		const rowWidth =
			TOP_LEVEL_DESCRIPTION_COLUMN + iconWidth + descriptionWidth;
		return Math.max(widest, rowWidth);
	}, visibleWidth("Menu"));
	return Math.max(
		TOP_LEVEL_MIN_WIDTH,
		Math.min(TOP_LEVEL_MAX_WIDTH, widestRow + TOP_LEVEL_EXTRA_WIDTH),
	);
}
