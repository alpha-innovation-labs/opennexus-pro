...
export function calculateModelMenuWidth(leaves: SlashMenuLeaf[]): number {
	const widestRow = leaves.reduce((widest, leaf) => {
		...
		const fixedLabelWidth = (leaf as { fixedLabelWidth?: number })
			.fixedLabelWidth;
		const labelWidth = fixedLabelWidth ?? visibleWidth(leaf.label);
		const descriptionWidth = leaf.description
			? visibleWidth(leaf.description) + 1
			: 0;
	...
	return Math.max(
		...
		Math.min(MODEL_MENU_MAX_WIDTH, widestRow + MODEL_MENU_EXTRA_WIDTH),
	);
}
