 ...
 */
export function createResourceCommandPreviewLines(
	item: SlashMenuLeaf | SlashMenuSection,
	theme: SharedModalTheme,
): string[] {
	return renderMarkdownPreview({
		markdown: createResourceCommandMarkdown(item),
		width: RESOURCE_COMMAND_PREVIEW_WIDTH,
		theme: {
			style: (token, value) => styleResourceMarkdownToken(theme, token, value),
		},
	});
}

/** Maps markdown preview tokens to Nexus theme colors for resource previews. */
function styleResourceMarkdownToken(
	theme: SharedModalTheme,
	...
	if (
		token === "lineNumber" ||
		token === "lineNumberSeparator" ||
		token === "inlineCode"
	)
		return value;
	if (token === "frontmatter.key" || token === "strikethrough")
		return theme.fg("error", value);
	if (token === "list.marker") return value;
	if (token === "frontmatter.value" || token === "link")
		return theme.fg("syntaxType", value);
	if (token === "frontmatter.marker") return theme.fg("warning", value);
	if (token === "frontmatter.border" || token === "horizontalRule")
