...
export function handleSlashMenuInput(input: {
  ...
  if (matchesKey(input.data, Key.enter) || matchesKey(input.data, Key.up) || matchesKey(input.data, Key.down) || matchesKey(input.data, Key.ctrl("n")) || matchesKey(input.data, Key.ctrl("p"))) {
    input.delegateInput();
    return true;
  }
  if (input.level === "scoped-models" && (input.data === "s" || matchesKey(input.data, Key.ctrl("s")))) {
    input.onCommandPicked(`/nexus-scoped-models-save ${encodeSlashMenuValue([...input.scopedSelection].join("\n"))}`);
    return true;
  }
