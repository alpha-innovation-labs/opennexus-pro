...
const PROJECT_SOURCE_NAMES = new Set(["project", "user", "temporary", "top-level"]);
 ...
 */
export function formatToolGroupLabel(sourceInfo: ToolSourceInfo | undefined): string {
...
function selectExtensionSource(sourceInfo: ToolSourceInfo): string {
  const source = sourceInfo.source ?? "";
  const path = sourceInfo.path ?? "";
  if (path.length > 0 && (PROJECT_SOURCE_NAMES.has(source) || source.length === 0)) return selectPathSegment(path);
...
function selectPathSegment(path: string): string {
  ...
  if (vendorIndex >= 0 && segments[vendorIndex + 1]) return segments[vendorIndex + 1] as string;
  const srcIndex = segments.lastIndexOf("src");
  if (srcIndex >= 0 && segments[srcIndex + 1]) return segments[srcIndex + 1] as string;
