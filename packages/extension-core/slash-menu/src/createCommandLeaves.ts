		...
		.filter(
			(command) =>
				![
					...
					"scoped-models",
					"settings",
				].includes(command.name) && command.hidden !== true,
		)
		.filter(
			(command) =>
				!isHiddenRuntimeSlashCommand(
					command.name,
					defaultRuntimeSlashCommandFilterConfig,
				),
		)
		.map((command) => ({
			kind: "command",
			label: command.name,
			description:
				normalizeBuiltinCommandDescription(command.name, command.description) ??
				"No description",
