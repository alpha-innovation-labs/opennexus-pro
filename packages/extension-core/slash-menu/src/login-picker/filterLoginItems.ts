...
export function filterLoginItems(
	...
	filteredProviders: Array<SlashMenuLeaf | SlashMenuSection>;
	filteredModelsByProvider: Map<string, SlashMenuLeaf[]>;
} {
	const _tokens = query.toLowerCase().trim();
	...
	for (const provider of allProviders) {
		...
		const filteredModels = filterMenuItems(
			providerModels as Array<SlashMenuLeaf | SlashMenuSection>,
			query,
		);
		const leafOnly = filteredModels.filter(
			(item): item is SlashMenuLeaf => "kind" in item,
		);
