...
export function updateResumePreview(input: {
	...
	const dialogWidth = Math.max(
		80,
		Math.min(input.width, Math.floor(input.width * 0.9)),
	);
	const innerWidth = Math.max(78, dialogWidth - 2);
	const leftPaneRatio =
		input.leftPaneRatio ?? (input.isRightPaneFocused() ? 0.3 : 0.6);
	...
	queueMicrotask(() => {
		try {
			const lines = readResumeTranscriptLines(
				input.ctx.ui.theme,
				previewWidth,
				input.item.value,
			);
			...
			if (
				requestId !== input.state.previewRequestId ||
				!input.isStillSelected(input.item)
			)
				return;
