...
export function registerSubagentReadTool(pi: ExtensionAPI): void {
	pi.registerTool(
		defineTool({
			name: "subagent_read",
			label: "Subagent Read",
			description: "Read terminal output from an agent via herdr agent read.",
			promptSnippet: "Read terminal output from a subagent",
			parameters: Type.Object({
				agentName: Type.String({
					description: "The target agent name (e.g. 'agent-abc1').",
				}),
				lines: Type.Optional(
					Type.Number({ description: "Optional number of lines to read." }),
				),
				source: Type.Optional(
					Type.String({
						description: "Optional source: 'visible' or 'recent'.",
					}),
				),
			}),
			async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
				try {
					...
					const text =
						typeof result === "object" && result !== null && "_raw" in result
