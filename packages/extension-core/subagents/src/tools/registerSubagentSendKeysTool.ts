...
export function registerSubagentSendKeysTool(pi: ExtensionAPI): void {
	pi.registerTool(
		defineTool({
			...
			description: "Send key presses (e.g. Enter, Esc) to a subagent.",
			promptSnippet: "Send key presses to a subagent",
			parameters: Type.Object({
				agentName: Type.String({
					description: "The target agent name (e.g. 'agent-abc1').",
				}),
				keys: Type.Array(Type.String(), {
					description: "Key presses to send (e.g. 'Enter', 'Esc').",
				}),
			}),
			async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
				try {
					...
					const message =
						(err as Error).message ?? `Send-keys failed: status=1`;
					return {
