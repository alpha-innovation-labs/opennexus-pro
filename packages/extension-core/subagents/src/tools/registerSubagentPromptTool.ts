...
export function registerSubagentPromptTool(pi: ExtensionAPI): void {
  pi.registerTool(defineTool({
    ...
    promptSnippet:
      "Send a prompt to a subagent and wait for the response",
    parameters: Type.Object({
      sessionId: Type.String({ description: "The agent session name (e.g. 'agent-abc1')." }),
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
      ...
      try {
        const result = promptHerdrAgent(params.sessionId, params.text, { timeoutMs: timeout });
