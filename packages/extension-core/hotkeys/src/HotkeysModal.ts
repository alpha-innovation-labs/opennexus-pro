...
import { getHotkeysConflict } from "./getHotkeysConflict";
import { getHotkeysEditableEntries } from "./getHotkeysEditableEntries";
import { getHotkeysEditStart } from "./getHotkeysEditStart";
import { getHotkeysFocusableEntries } from "./getHotkeysFocusableEntries";
import { getHotkeysGroups } from "./getHotkeysGroups";
import { getHotkeysScrollTarget } from "./getHotkeysScrollTarget";
import { getHotkeysVisibleLineCount } from "./getHotkeysVisibleLineCount";
import { getNextHotkeysFocus } from "./getNextHotkeysFocus";
import { renderHotkeysConflictModal } from "./renderHotkeysConflictModal";
import { renderHotkeysLines } from "./renderHotkeysLines";
import { resolveHotkeysFocus } from "./resolveHotkeysFocus";
import { resolveHotkeysPaneFocus } from "./resolveHotkeysPaneFocus";
import { saveHotkeysBinding } from "./saveHotkeysBinding";
import type {
...
export class HotkeysModal extends SharedModal {
	...
	constructor(
		...
		private readonly keybindings: HotkeysKeybindings,
		private readonly extensionShortcuts: HotkeysExtensionShortcut[],
		onClose: () => void,
	) {
		super({
			theme: uiTheme,
			...
			maxWidthRatio: 1,
			fullScreen: true,
			fullScreenRows: () => process.stdout.rows || 40,
			onClose,
			headerLines: [uiTheme.fg("accent", "● Hotkeys")],
			panes: [],
		});
	...
	private handleFilterInput(data: string): void {
		...
		if (result.filterActive !== undefined)
			this.filterActive = result.filterActive;
	...
	private moveFocus(direction: -1 | 1): void {
		const entries = getHotkeysEditableEntries(
			this.keybindings,
			this.extensionShortcuts,
			this.filterQuery,
		);
		this.focusedEntryId = getNextHotkeysFocus(
			entries,
			resolveHotkeysFocus(entries, this.focusedEntryId),
			direction,
		);
		this.statusMessage = "";
	}
	...
	private movePaneFocus(direction: -1 | 1): void {
		const groups = filterHotkeysGroups(
			getHotkeysGroups(this.keybindings, this.extensionShortcuts),
			this.filterQuery,
		);
		this.focusedEntryId = resolveHotkeysPaneFocus(
			groups,
			this.focusedEntryId,
			direction,
		);
	...
	private startEditingFirstMatch(): void {
		const entries = getHotkeysFocusableEntries(
			this.keybindings,
			this.extensionShortcuts,
			this.filterQuery,
		);
		const editStart = getHotkeysEditStart(
			entries,
			resolveHotkeysFocus(entries, this.focusedEntryId),
		);
	...
	private handleEditInput(data: string): void {
		...
		const conflictingIds = getHotkeysConflict(
			this.keybindings,
			keybindingId,
			key,
		);
	...
	private saveEditedKeybinding(
		keybindingId: string,
		key: string,
		overriddenKeybindingIds: string[] = [],
	): void {
		const configPath = saveHotkeysBinding(
			this.keybindings,
			keybindingId,
			key,
		...
		this.statusMessage = configPath
			? `Saved ${keybindingId} = ${key}`
			: `Updated ${keybindingId} = ${key}`;
	}
	...
	render(width: number): string[] {
		...
		const focusEntries = this.focusedEntryId
			? getHotkeysFocusableEntries(
					this.keybindings,
					this.extensionShortcuts,
					this.filterQuery,
				)
			: getHotkeysEditableEntries(
					this.keybindings,
					this.extensionShortcuts,
					this.filterQuery,
				);
		this.focusedEntryId = resolveHotkeysFocus(
			focusEntries,
			this.focusedEntryId,
		);
		const lines = renderHotkeysLines(
			this.uiTheme,
			groups,
			dialogWidth,
			this.focusedEntryId,
		...
		if (this.pendingConflict)
			lines.unshift(
				...renderHotkeysConflictModal(
					this.uiTheme,
					this.pendingConflict,
					dialogWidth,
				),
			);
		const visibleLineCount = getHotkeysVisibleLineCount(
		...
		this.scrollOffset = clampHotkeysScrollOffset(
			this.scrollOffset,
			lines.length,
			visibleLineCount,
		);
		const footerState = createHotkeysModalFooterState(
			Boolean(this.pendingConflict),
			this.editingEntryId,
			this.statusMessage,
			this.filterActive,
			this.filterQuery,
		...
		this.footerLines = footerState.footerLines.map((line) =>
			this.uiTheme.fg("dim", line),
		);
		const visibleLines = lines.slice(
			this.scrollOffset,
			this.scrollOffset + visibleLineCount,
		);
