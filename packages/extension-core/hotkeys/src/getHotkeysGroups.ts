 ...
 */
export function getHotkeysGroups(
	keybindings: HotkeysKeybindings,
	extensionShortcuts: HotkeysExtensionShortcut[] = getRegisteredHotkeysShortcuts(),
): HotkeysGroup[] {
	const groups = createHotkeysGroupMap(GROUP_ORDER);
	for (const group of getStaticHotkeysGroups()) groups.set(group.title, group);

	for (const [keybinding, value] of Object.entries(
		getResolvedHotkeysBindings(keybindings),
	)) {
