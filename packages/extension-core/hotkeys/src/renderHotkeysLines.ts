 ...
 */
export function renderHotkeysLines(uiTheme: SelectPreviewTheme, groups: HotkeysGroup[], dialogWidth: number, focusedEntryId?: string, editingEntryId?: string): string[] {
  const gap = 2;
  const columnWidth = Math.floor((dialogWidth - gap) / 2);
  const [leftGroups, rightGroups] = arrangeFocusedHotkeysGroups(groups, focusedEntryId);
  const leftLines = renderHotkeysColumn(uiTheme, leftGroups, columnWidth, focusedEntryId, editingEntryId);
  const rightLines = renderHotkeysColumn(uiTheme, rightGroups, dialogWidth - columnWidth - gap, focusedEntryId, editingEntryId);
  ...
  if (groups.length === 0) lines.push(uiTheme.fg("dim", "No matching keybindings"));
  return lines;
}
