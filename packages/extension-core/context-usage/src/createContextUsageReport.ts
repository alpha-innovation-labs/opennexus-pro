 ...
 */
function createFallbackBaseSystemPromptText(
	options: ContextUsageRuntimeSnapshot["systemPromptOptions"],
): string {
	const guidelines = [
...
function mergeToolItems(
	primary: ContextUsageReport["systemTools"],
	fallback: ContextUsageReport["systemTools"],
): ContextUsageReport["systemTools"] {
 ...
 */
export async function createContextUsageReport(
	snapshot: ContextUsageRuntimeSnapshot,
	...
	const structuredMcpTools = createToolItems(
		snapshot.systemPromptOptions?.toolSnippets,
		true,
	);
	const structuredSystemTools = createToolItems(
		snapshot.systemPromptOptions?.toolSnippets,
		false,
	);
	const piBuiltinTools = await createPiBuiltinToolItems(
		snapshot.systemPromptOptions?.cwd ?? process.cwd(),
	);
	const mcpTools =
		promptMcpTools.length > 0 ? promptMcpTools : structuredMcpTools;
	const systemTools =
		promptSystemTools.length > 0
			? promptSystemTools
			: mergeToolItems(structuredSystemTools, piBuiltinTools);
	const agentsFiles = createAgentsItems(
	...
	const skills =
		structuredSkills.length > 0
			? structuredSkills
			: createPromptSkillItems(snapshot.systemPrompt);
	const knownSystemTokens =
	...
	const fallbackBaseSystemPromptTokens = estimateTokensFromText(
		createFallbackBaseSystemPromptText(snapshot.systemPromptOptions),
	);
	const systemPromptTokens = Math.max(
		0,
		Math.max(
			renderedSystemPromptTokens,
			knownSystemTokens + fallbackBaseSystemPromptTokens,
		) - knownSystemTokens,
	);
	const messagesTokens = estimateTokensFromText(
	...
	return {
		...
		contextWindow,
		usedPercent:
			snapshot.usage?.percent ?? calculatePercent(usedTokens, contextWindow),
		categories: [
			{
				marker: "⛁",
				label: "System prompt",
				tokens: systemPromptTokens,
				percent: calculatePercent(systemPromptTokens, contextWindow),
			},
			{
				marker: "⛁",
				label: "System tools",
			...
			},
			{
				...
				percent: calculatePercent(sumTokens(mcpTools), contextWindow),
			},
			{
				marker: "⛁",
				label: "AGENTS.md",
				tokens: sumTokens(agentsFiles),
				percent: calculatePercent(sumTokens(agentsFiles), contextWindow),
			},
			{
				marker: "⛁",
				label: "Skills",
				tokens: sumTokens(skills),
				percent: calculatePercent(sumTokens(skills), contextWindow),
			},
			{
				marker: "⛁",
				label: "Messages",
			...
			},
			{
				...
				label: "Free space",
				tokens: freeTokens,
				percent: calculatePercent(freeTokens, contextWindow),
			},
			{
				marker: "⛝",
				label: "Autocompact buffer",
				tokens: reserveTokens,
				percent: calculatePercent(reserveTokens, contextWindow),
			},
		],
		systemTools,
		mcpTools,
