...
import type {
	GrepSearchResponse,
	ResolvedPath,
	StoredGrepContinuation,
} from "../shared/types";
import { buildGrepOutput } from "./buildGrepOutput";
import { combineConstraints } from "./combineConstraints";
import { createFinder } from "./createFinder";
import { createFinderError } from "./createFinderError";
import { createRequestKey } from "./createRequestKey";
import { getRuntimePaths } from "./getRuntimePaths";
import { loadFffNode } from "./loadFffNode";
import { nativeConstraintForGlob } from "./nativeConstraintForGlob";
import { nativeConstraintForScope } from "./nativeConstraintForScope";
import { normalizeCandidate } from "./normalizeCandidate";
import { normalizePathQuery } from "./normalizePathQuery";
import { resolveExistingPath } from "./resolveExistingPath";
import { resolveProjectRoot } from "./resolveProjectRoot";
import { resolveTrackedPath } from "./resolveTrackedPath";
import { shouldAutoResolveCandidate } from "./shouldAutoResolveCandidate";
import { stripPathLocation } from "./stripPathLocation";
...
export class FffRuntime {
	...
	private readonly grepContinuations = new Map<
		string,
		StoredGrepContinuation
	>();
	...
	async searchFileCandidates(query: string, limit = DEFAULT_FILE_LIMIT) {
		...
		const search = finder.fileSearch(normalizedQuery, {
			pageSize: Math.max(limit, DEFAULT_FILE_LIMIT),
		});
	...
	async trackQuery(query: string, selectedPath: string): Promise<void> {
		...
		this.projectRoot = projectRoot;
		const trackedPath = resolveTrackedPath(
			projectRoot,
			this.cwd,
			selectedPath,
		).replace(/\\/g, "/");
	...
	async resolvePath(
		...
		const direct = await resolveExistingPath(
			this.cwd,
			basePath,
			pathOnlyQuery,
			allowDirectory,
		...
		const candidates = search.value.items
			...
			.filter(
				(candidate) =>
					allowDirectory || !candidate.item.relativePath.endsWith("/"),
			);
		if (!shouldAutoResolveCandidate(candidates[0], candidates[1])) {
			const formatted = candidates
				.map(
					(candidate, index) => `${index + 1}. ${candidate.item.relativePath}`,
				)
				.join("\n");
			throw new Error(
				formatted
					? `Could not resolve "${query}" uniquely.\nTop matches:\n${formatted}`
	...
	async grepSearch(args: {
		...
		const finder = await this.ensure();
		const scope = args.pathQuery
			? await this.resolvePath(args.pathQuery, true)
			: undefined;
		const constraintQuery = combineConstraints(
			nativeConstraintForScope(scope),
			nativeConstraintForGlob(args.glob),
		);
		const requestKey = createRequestKey({ ...args, constraintQuery });
		const continuation = args.cursor
			? this.grepContinuations.get(args.cursor)
			: undefined;
		...
		if (continuation && continuation.requestKey !== requestKey) {
			throw new Error(
				"This grep cursor belongs to a different query. Re-run without cursor.",
			);
		}

		const request = finder.grep(
			constraintQuery ? `${constraintQuery} ${args.pattern}` : args.pattern,
			{
				mode: args.mode,
		...
		const nextCursor =
			remainingItems.length > 0 || request.value.nextCursor !== null
				? this.storeContinuation({
						requestKey,
						remainingItems,
						engineCursor: request.value.nextCursor,
					})
				: undefined;
