...
export function registerGrepOverride(pi: ExtensionAPI): void {
	...
	pi.registerTool({
		...
		promptGuidelines: (template as { promptGuidelines?: string[] })
			.promptGuidelines,
		...
		async execute(toolCallId, params, signal, onUpdate, ctx) {
			...
			if (
				!enabledFeatures.has("grepOverride") ||
				shouldFallbackToBuiltinGrep(builtinParams)
			) {
			...
			try {
				...
				return {
					content: [
						{ type: "text" as const, text: buildGrepFailureMessage(error) },
					],
					details: {
