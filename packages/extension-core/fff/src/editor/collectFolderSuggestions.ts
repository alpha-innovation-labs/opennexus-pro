 ...
 */
export function collectFolderSuggestions(candidates: FffFileCandidate[], rawQuery: string): string[] {
  const normalizedQuery = rawQuery.replace(/^\.?\//, "").replace(/\\/g, "/").toLowerCase();
  ...
  for (const candidate of candidates) {
    ...
    for (let index = 1; index < parts.length; index += 1) {
      ...
      if (normalizedQuery && !normalizedFolderPath.includes(normalizedQuery) && !folderName.includes(normalizedQuery)) {
        continue;
      }
      seen.add(folderPath);
  ...
  folders.sort((left, right) => left.length - right.length || left.localeCompare(right));
  return folders;
}
