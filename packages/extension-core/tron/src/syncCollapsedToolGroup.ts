 ...
 */
export function syncCollapsedToolGroup(
	toolCallIds: string[],
	assistantTimestamp?: number,
): void {
	const uniqueToolCallIds = toolCallIds.filter(
		(toolCallId, index) =>
			toolCallId && toolCallIds.indexOf(toolCallId) === index,
	);
	...
	for (const toolCallId of uniqueToolCallIds) {
		const previousLeaderToolCallId =
			collapsedToolGroupLeaderByToolCallId.get(toolCallId);
		if (
			!previousLeaderToolCallId ||
			previousLeaderToolCallId === leaderToolCallId
		)
			continue;
		const previousStats = collapsedToolGroupStatsByLeader.get(
			previousLeaderToolCallId,
		);
		...
		for (const [
			currentToolCallId,
			toolName,
		] of previousStats.toolNames.entries())
			stats.toolNames.set(currentToolCallId, toolName);
		for (const currentToolCallId of previousStats.countedToolCallIds.values())
			stats.countedToolCallIds.add(currentToolCallId);
		...
		if (typeof previousStats.startedAt === "number") {
			stats.startedAt =
				typeof stats.startedAt === "number"
					? Math.min(stats.startedAt, previousStats.startedAt)
					: previousStats.startedAt;
		}
		if (typeof previousStats.finishedAt === "number") {
			stats.finishedAt =
				typeof stats.finishedAt === "number"
					? Math.max(stats.finishedAt, previousStats.finishedAt)
					: previousStats.finishedAt;
		}
		if (typeof previousStats.firstAssistantTimestamp === "number") {
			stats.firstAssistantTimestamp =
				typeof stats.firstAssistantTimestamp === "number"
					? Math.min(
							stats.firstAssistantTimestamp,
		...
		if (typeof previousStats.lastAssistantTimestamp === "number") {
			stats.lastAssistantTimestamp =
				typeof stats.lastAssistantTimestamp === "number"
					? Math.max(
							stats.lastAssistantTimestamp,
	...
	if (typeof assistantTimestamp === "number") {
		stats.firstAssistantTimestamp = Math.min(
			stats.firstAssistantTimestamp ?? assistantTimestamp,
			assistantTimestamp,
		);
		stats.lastAssistantTimestamp = Math.max(
