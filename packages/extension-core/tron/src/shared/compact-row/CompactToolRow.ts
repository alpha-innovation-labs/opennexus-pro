...
export class CompactToolRow {
	...
	render(): string[] {
		...
		lines.push(
			`${theme.fg("borderMuted", "│")}${renderCompactLine({ width: innerWidth, icon, label, main, inlineStats, renderedInlineStats, options, renderedOptions, theme })}${theme.fg("borderMuted", "│")}`,
		);
		if (showBottomBorder) {
			lines.push(theme.fg("borderMuted", `└${"─".repeat(innerWidth)}┘`));
		}
