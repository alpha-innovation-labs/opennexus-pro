...
export function renderCompactLine(params: {
	...
	const naturalWidth =
		leftPrefixWidth + naturalGapWidth + mainNaturalWidth + optionsNaturalWidth;
	const availableAfterPrefix = Math.max(0, safeWidth - leftPrefixWidth - 1);
	const cappedOptionsWidth = Math.min(
		optionsNaturalWidth,
		Math.max(0, Math.floor(safeWidth * 0.35)),
		availableAfterPrefix,
	);
	const maxOptionsWidth = options
		? naturalWidth <= safeWidth
			? optionsNaturalWidth
			: cappedOptionsWidth
		: 0;
	const shownOptions = options
		? truncateToWidth(options, maxOptionsWidth, "…")
		: "";
	const optionsWidth = visibleWidth(shownOptions);
	const optionsGap = shownOptions ? 1 : 0;
	const maxMainWidth = main
		? Math.max(0, safeWidth - leftPrefixWidth - optionsGap - optionsWidth - 1)
		: 0;
	...
	const gapWidth = Math.max(
		0,
		safeWidth - leftWidth - optionsGap - optionsWidth,
	);
	const padWidth = Math.max(
		0,
		safeWidth - leftWidth - gapWidth - optionsGap - optionsWidth,
	);
	...
	return (
		...
		" ".repeat(gapWidth) +
		(shownOptions
			? ` ${createRenderedOptions(shownOptions, renderedOptions, theme)}`
			: "") +
		" ".repeat(padWidth)
	);
}
