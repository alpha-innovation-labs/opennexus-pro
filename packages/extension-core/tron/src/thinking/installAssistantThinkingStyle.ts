 ...
 */
function isVisibleToolCall(content: unknown): boolean {
...
export function installAssistantThinkingStyle(): void {
	setAssistantMessageUpdateHook((component: unknown, message: unknown) => {
		...
		const hasVisibleToolCalls = message.content.some((content: unknown) =>
