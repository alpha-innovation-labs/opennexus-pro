...
export function renderTranscriptLines(
	...
	const frameContent: FrameContent[] = run.transcript.flatMap((entry) => {
		if (entry.role === "thinking")
			return [{ type: "thinking", thinking: entry.text }] as FrameContent[];
		if (entry.role === "tool") {
			return [
				{
					type: "toolCall",
					id:
						entry.toolCallId ??
						`${entry.toolName ?? "tool"}-${entry.createdAt}`,
					name: entry.toolName,
				},
			] as FrameContent[];
		}
	...
	return run.transcript.flatMap((entry, index) => {
		...
		const connectThinkingToTools =
			entry.role === "thinking" && nextEntry?.role === "tool";
		const connectThinkingFromTool =
			entry.role === "thinking" && previousEntry?.role === "tool";
