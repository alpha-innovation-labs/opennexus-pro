...
export class AgentWidget {
  ...
  private renderFinishedLine(width: number, a: { id: string; type: SubagentType; status: string; description: string; toolUses: number; startedAt: number; completedAt?: number; error?: string }, theme: Theme): string {
    ...
    const parts: string[] = [];
    if (a.toolUses > 0) parts.push(`${a.toolUses} tools`);
    parts.push(duration);
