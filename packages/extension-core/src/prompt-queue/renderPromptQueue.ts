 ...
 * @param editing Whether a queued item is being edited.
 * @param _isPendingDispatch Retained for queue dispatch compatibility; no pending status text is rendered.
 * @returns Rendered prompt queue lines.
 */
export function renderPromptQueue(width: number, items: PromptQueueItem[], selectedId: string | undefined, focused: boolean, theme: ExtensionContext["ui"]["theme"], hasMessages = true, editing = false, _isPendingDispatch: (id: string) => boolean = () => false): string[] {
  ...
  for (const item of visibleItems) {
    ...
    const text = formatPromptQueuePreview(item.text, Math.max(1, innerWidth - 3));
