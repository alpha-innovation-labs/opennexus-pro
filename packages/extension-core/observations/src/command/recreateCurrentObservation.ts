 ...
 */
export async function recreateCurrentObservation(ctx: ExtensionContext | ExtensionCommandContext): Promise<boolean> {
	...
	const result = await runObservationRecreateCliCommand(ctx.sessionManager.getSessionId(), ctx.cwd);
	if (result.exitCode === 0) {
		ctx.ui.notify(result.stdout.trim() || "Recreated observations", "info");
		return true;
	}

	ctx.ui.notify(result.stderr.trim() || "Failed to recreate observations", "error");
	return false;
}
