export type TopicDecision = { action: "keep" } | { action: "new_topic"; title: string };
 ...
 */
export function parseTopicDecisionOutput(output: string): TopicDecision | undefined {
	...
	try {
		...
		if (candidate.action !== "new_topic" || typeof candidate.title !== "string") return undefined;
