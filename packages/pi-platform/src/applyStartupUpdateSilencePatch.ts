...
export function applyStartupUpdateSilencePatch(): void {
	...
	prototype.checkForNewVersion = async function checkForNewVersion(): Promise<
		string | undefined
	> {
		return undefined;
	};
	prototype.checkForPackageUpdates =
		async function checkForPackageUpdates(): Promise<string[]> {
			return [];
		};
	prototype.showNewVersionNotification = function showNewVersionNotification(
		_newVersion: string,
	): void {
		return;
	};
	prototype.showPackageUpdateNotification =
		function showPackageUpdateNotification(_packages: string[]): void {
			return;
		};
