...
export function applyCompactModeImagePatch(): void {
	...
	const prototype =
		ToolExecutionComponent.prototype as unknown as ToolExecutionComponentWithCompactImagePatch;
	const originalUpdateDisplay = prototype.updateDisplay;
	const originalGetTextOutput = prototype.getTextOutput;

	prototype.updateDisplay =
		function updateDisplayWithoutCompactModeImages(): void {
	...
	prototype.getTextOutput =
		function getTextOutputWithoutCompactModeImages(): string {
			if (!isToolGroupCollapseEnabled())
				return originalGetTextOutput.call(this);
			const textBlocks =
				this.result?.content?.filter((block) => block?.type === "text") ?? [];
			return textBlocks
				.map((block) =>
					sanitizeBinaryOutput(stripAnsi(block.text || "")).replace(/\r/g, ""),
				)
				.join("\n");
		};
