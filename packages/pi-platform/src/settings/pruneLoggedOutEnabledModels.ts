...
export async function pruneLoggedOutEnabledModels(cwd: string): Promise<void> {
	...
	const settings = SettingsManager.create(
		cwd,
		getNexusAgentDir(),
	) as unknown as WritableSettingsManager;
	const enabledModels = settings.getEnabledModels();
	const nextEnabledModels = filterLoggedInEnabledModelPatterns(
		enabledModels,
		authStorage,
	);
	if (
		enabledModels === nextEnabledModels ||
		areEqualModels(enabledModels, nextEnabledModels)
	)
		return;
