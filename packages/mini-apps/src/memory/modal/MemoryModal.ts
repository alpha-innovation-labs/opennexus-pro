...
export class MemoryModal extends SelectPreviewModal {
	private expandedReferences = new Set<string>();
	private filterQuery = "";
	...
	constructor(theme: ExtensionCommandContext["ui"]["theme"], items: MemoryItem[], done: (result: undefined) => void, private readonly onDelete: (item: MemoryItem) => Promise<void>) {
		super(theme, () => this.toggleSelectedReferences(), () => done(undefined), undefined, { fullScreen: true, leftTitle: "Memory", rightTitle: "Markdown", leftPaneRatio: 0.38, bottomTitle: "Keys" });
		this.memoryItems = items;
		this.treeRows = createMemoryFileTreeItems(items, this.expandedReferences);
		this.setOnSelectionChange((item) => this.setRightLines(this.getPreviewLines(item?.value)));
		this.refreshItems();
	}

	/** Handles search, delete, toggle, and list navigation input. */
	override handleInput(data: string): void {
		if (data === "/" && !this.searchActive) return this.startSearch();
		if (data === "d" && !this.searchActive) return void this.deleteSelected();
		if ((data === "l" || matchesKey(data, Key.enter)) && !this.searchActive && this.toggleSelectedReferences()) return;
	...
	private startSearch(): void { this.searchActive = true; this.refreshBottom(); }
	...
	/** Toggles the selected references folder. */
	private toggleSelectedReferences(): boolean {
		const row = this.getSelectedRow();
		if (row?.role !== "references" || !row.projectSlug) return false;
		if (this.expandedReferences.has(row.projectSlug)) this.expandedReferences.delete(row.projectSlug);
		else this.expandedReferences.add(row.projectSlug);
		this.rebuildRows(row.path);
		return true;
	}
	...
	private handleSearchInput(data: string): boolean {
		...
		if (matchesKey(data, Key.escape)) { this.searchActive = false; this.filterQuery = ""; this.refreshItems(); return true; }
	...
	/** Rebuilds rows from memory items and expansion state. */
	private rebuildRows(selectedPath?: string): void {
		this.treeRows = createMemoryFileTreeItems(this.memoryItems, this.expandedReferences);
		this.refreshItems(selectedPath);
	}

	/** Refreshes list items and footer from the active filter. */
	private refreshItems(selectedPath?: string): void {
		const visible = filterMemoryTreeItems(this.treeRows, this.filterQuery);
		this.setItems(createMemoryTreeModalItems(visible));
		if (selectedPath) this.selectValue(selectedPath);
		this.refreshBottom();
		this.setRightLines(this.getPreviewLines(selectedPath ?? visible[0]?.path));
	}

	/** Refreshes the footer with current mode hints. */
	private refreshBottom(): void {
		const value = this.searchActive ? `/${this.filterQuery}` : "j/k/h/l or arrows move · enter/l references · / search · d delete · esc close";
		this.setBottom(this.searchActive ? "Search" : "Keys", value, this.searchActive ? "" : "");
	}

	/** Returns the current selected tree row. */
	private getSelectedRow(): MemoryTreeItem | undefined { return this.treeRows.find((row) => row.path === this.getSelectedItem()?.value); }
	...
	private getPreviewLines(path: string | undefined): string[] {
		...
		if (row?.role === "references") return this.getReferenceListLines(row.projectSlug);
		return row?.item ? [row.item.path, "", ...row.item.content.split(/\r?\n/)] : ["No memory item selected"];
	}

	/** Builds the right-pane list for a references folder. */
	private getReferenceListLines(projectSlug: string | undefined): string[] {
		const references = this.memoryItems.filter((item) => projectSlug && item.relativePath.startsWith(`${projectSlug}/references/`));
		return ["References", "", ...references.map((item) => `- ${item.relativePath.split("/").at(-1) ?? item.relativePath}`)];
	}
}
