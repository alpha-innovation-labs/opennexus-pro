 ...
 */
export function createCompactTetrisModalLines(
	...
	const scoreHeight =
		height >= 16 ? 7 : Math.min(3, Math.max(0, height - hotkeysHeight));
	const nextHeight =
		height >= 18
			? Math.min(5, Math.max(0, height - scoreHeight - hotkeysHeight - 3))
			: 0;
	const boardHeight = Math.max(
		0,
		height - scoreHeight - nextHeight - hotkeysHeight,
	);
	const sections = [
		...renderTetrisStatsLineBox(theme, game, sectionWidth, musicPlaying).slice(
			0,
			scoreHeight,
		),
	...
	return padTetrisLines(
		sections.map((line) => padTetrisLine(line, width)),
		height,
	).slice(0, height);
}
