...
import {
	getTetrisRenderCells,
	type TetrisRenderCell,
} from "../game/getTetrisRenderCells";
 ...
 */
export function renderScaledTetrisBoard(
	theme: SharedModalTheme & { bold: (text: string) => string },
	game: TetrisGame,
	width: number,
	height: number,
	options: { fillWidth?: boolean; title?: string } = {},
): string[] {
	const defaultCellWidth = getTetrisCellWidth(
		width,
		height,
		game.width,
		game.height,
	);
	const boardInnerWidth = options.fillWidth
		? Math.max(game.width, width - 2)
		: defaultCellWidth * game.width;
	const columnWidths = options.fillWidth
		? createTetrisSegmentWidths(boardInnerWidth, game.width)
		: Array.from({ length: game.width }, () => defaultCellWidth);
	const title = options.title ? ` ${options.title} ` : "";
	const topBorder = centerTetrisLine(
		theme.fg(
			"borderMuted",
			`┌${title}${"─".repeat(Math.max(0, boardInnerWidth - title.length))}┐`,
		),
		width,
	);
	const bottomBorder = centerTetrisLine(
		theme.fg("borderMuted", `└${"─".repeat(boardInnerWidth)}┘`),
		width,
	);
	const rows = getTetrisRenderCells(game).map((row) =>
		centerTetrisLine(renderScaledTetrisRow(theme, row, columnWidths), width),
	);
	...
	if (game.gameOver && clippedRows.length > 0) {
		...
		clippedRows[overlayRow] = centerTetrisLine(
			theme.fg("error", theme.bold(" Game Over! ")),
			width,
		);
	}
 ...
 */
function renderScaledTetrisRow(
	theme: SharedModalTheme & { bold: (text: string) => string },
	row: TetrisRenderCell[],
	columnWidths: number[],
): string {
	const content = row
		.map((cell, index) =>
			renderTetrisCellSegment(theme, cell, columnWidths[index] ?? 1),
		)
		.join("");
