 ...
 */
export function createWideTetrisModalLines(
	...
	const preferredBoardWidth = Math.max(
		34,
		Math.floor((game.width * 3 + 2) * 0.85),
	);
	const boardWidth = Math.max(
		34,
		Math.min(58, preferredBoardWidth, width - fixedWidth),
	);
	const contentWidth =
		scoreWidth + boardWidth + rightWidth + visibleWidth(gap) * 2;
	...
	const scoreLines = padTetrisLines(
		renderTetrisStatsBox(theme, game, scoreWidth, scoreHeight, musicPlaying),
		contentHeight,
	).map((line) => padTetrisLine(line, scoreWidth));
	const boardLines = padTetrisLines(
		renderTetrisBoardBox(theme, game, boardWidth, contentHeight),
		contentHeight,
	).map((line) => padTetrisLine(line, boardWidth));
	const rightLines = padTetrisLines(
		...
		contentHeight,
	).map((line) => padTetrisLine(line, rightWidth));
	return Array.from({ length: contentHeight }, (_, index) =>
		padTetrisLine(
			`${" ".repeat(leftPad)}${scoreLines[index] ?? ""}${gap}${boardLines[index] ?? ""}${gap}${rightLines[index] ?? ""}`,
			width,
		),
	);
}
