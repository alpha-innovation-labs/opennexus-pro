	...
	switch (op.kind) {
		...
		case "create_many": {
			...
			return created.length === 0
				? "Created 0 tasks"
				: `Created ${created.length} tasks\n${created.map((task) => `#${task.id}: ${task.subject} (pending)`).join("\n")}`;
		}
		case "update": {
		...
		case "delete_many":
			return op.deleted.length === 0
				? "Deleted 0 tasks"
				: `Deleted ${op.deleted.length} tasks\n${op.deleted.map((task) => `#${task.id}: ${task.subject}`).join("\n")}`;
		case "clear":
 ...
 *
 * Mirrors `packages/extension-core/src/ask-user-question/tool/response-envelope.ts:13-47`.
 */
