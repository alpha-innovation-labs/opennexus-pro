 ...
 * surface — it mirrors `packages/extension-core/src/ask-user-question/ask-user-question.ts`
...
export function registerTodoTool(pi: ExtensionAPI): void {
	pi.registerTool({
		...
		label: TOOL_LABEL,
		description:
			"Manage a task list for tracking multi-step progress. Actions: create/create_many (new tasks), update (change status/fields/dependencies), list (all tasks, optionally filtered by status), get (single task details), delete/delete_many (tombstone), clear (reset all). Status: pending → in_progress → completed, plus deleted tombstone. Use this to plan and track multi-step work like research, design, and implementation.",
		...
		promptGuidelines: [
			...
			"Use create_many with items to add several tasks in one call, and delete_many with ids to tombstone several tasks in one call.",
		],
