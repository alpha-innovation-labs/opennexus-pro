...
export function summarizeGenericObjectArgs(args: unknown): SummaryText {
  if (!args || typeof args !== "object" || Array.isArray(args)) {
    return { main: truncateSingleLine(firstLine(JSON.stringify(args)), 240), options: "" };
  }

  const record = args as Record<string, unknown>;
  const mainParts = MAIN_FIELD_NAMES
    ...
    .map(String)
    .filter(Boolean);
  const optionParts = OPTION_FIELD_NAMES
    .filter((fieldName) => !MAIN_FIELD_NAMES.includes(fieldName))
    .map((fieldName) => formatOptionField(fieldName, record[fieldName]))
    .filter(Boolean);

  const main = mainParts.length > 0 ? mainParts.join(" · ") : JSON.stringify(record);
  return {
