...
/**
 * Searches npm for discoverable Pi packages and maps them to extension-manager rows.
 *
 ...
 */
export async function fetchNpmPackageSearchRows(query: string, installedSources: Set<string>): Promise<ManagedExtensionRow[]> {
	...
	const data = await response.json() as {
		objects?: Array<{ package: { name: string; description?: string; version?: string; links?: { repository?: string; homepage?: string } } }>;
	};
	return (data.objects ?? []).flatMap((item) => {
		...
		return [{
			...
			features: [item.package.description ?? "", item.package.version ? `latest ${item.package.version}` : "npm package"],
			rowType: "search" as const,
			source,
			repository: item.package.links?.repository ?? item.package.links?.homepage,
		}];
	});
}
