 ...
 */
export function removeTriggerPrefixFromLines(lines: string[], cursorLine: number, cursorCol: number, prefix: string): string {
  if (!prefix) return lines.join("\n");
  const nextLines = [...lines];
  const line = nextLines[cursorLine] ?? "";
  const prefixStart = Math.max(0, cursorCol - prefix.length);
  nextLines[cursorLine] = `${line.slice(0, prefixStart)}${line.slice(cursorCol)}`;
