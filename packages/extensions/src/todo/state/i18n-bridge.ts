/**
 * i18n bridge for todo — single thin import surface so every call site
 ...
 *
 * - `t(key, fallback)` is `scope("@nexus/todo")` if the SDK is
 *   installed (live `/languages` updates propagate). If the SDK is missing
 *   (standalone install without i18n), `t` is an identity passthrough
...
export const I18N_NAMESPACE = "@nexus/todo";
