 ...
 */
export function renderSystemPromptOutline(outline: SystemPromptOutline, theme: SelectPreviewTheme): string[] {
	const lines: string[] = [];
	for (const [sectionIndex, section] of outline.sections.entries()) {
		...
		for (const [childIndex, child] of section.children.entries()) {
			const connector = renderOutlineConnector(childIndex === section.children.length - 1);
			lines.push(`${theme.fg("dim", sectionLast ? "  " : "│ ")}${theme.fg("dim", connector)} ${child.label}`);
		}
	}
	return lines;
}
