 ...
 */
export function createChartAxisLabels(rowCount: number, min: number, max: number, unit: UsageHistoryUnit): string[] {
  if (unit === "percent") return Array.from({ length: rowCount }, (_unused, index) => formatAxisLabel(100 - index * 5, unit));
