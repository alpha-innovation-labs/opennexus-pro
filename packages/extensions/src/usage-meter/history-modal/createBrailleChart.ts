 ...
 */
export function createBrailleChart(records: UsageHistoryRecord[], width: number, height: number): string[] {
  ...
  const plotted = points.map((point, index) => ({
    x: Math.round((index / Math.max(1, points.length - 1)) * pixelWidth),
