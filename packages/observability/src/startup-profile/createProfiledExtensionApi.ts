 ...
 */
export function createProfiledExtensionApi(pi: ExtensionAPI, extensionId: string): ExtensionAPI {
	...
	return new Proxy(pi, {
		get(target, property, receiver) {
			...
			return (eventName: string, handler: (...args: unknown[]) => unknown) =>
				target.on(eventName as never, wrapExtensionEventHandler(extensionId, eventName, handler) as never);
		},
	}) as ExtensionAPI;
}
