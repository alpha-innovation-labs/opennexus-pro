...
export function computePaneWidths(
	...
	const totalSize = panes.reduce(
		(sum, pane) => sum + Math.max(1, pane.size),
		0,
	);
	const widths = panes.map((pane) =>
		Math.max(
			pane.minWidth ?? 1,
			Math.floor((availableWidth * Math.max(1, pane.size)) / totalSize),
		),
	);
	const overflow =
		widths.reduce((sum, width) => sum + width, 0) - availableWidth;
