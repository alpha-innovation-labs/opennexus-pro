...
export function createTwoPaneHeaderLine(options: TwoPaneHeaderOptions): string {
	...
	const left = formatHeaderTitle(
		options.leftTitle,
		options.activePane === "left",
		options.uiTheme,
		showFocusMarkers,
	);
	const right = formatHeaderTitle(
		options.rightTitle,
		options.activePane === "right",
		options.uiTheme,
		showFocusMarkers,
	);
	if (
		options.showLeftPane &&
		options.showRightPane &&
		options.rightTitle.trim() === ""
	)
		return options.leftTitle;
	if (options.showLeftPane && options.showRightPane) {
		if (options.leftWidth !== undefined && options.rightWidth !== undefined) {
			const separator = showFocusMarkers
				? options.uiTheme.fg("borderMuted", "│")
				: " ";
