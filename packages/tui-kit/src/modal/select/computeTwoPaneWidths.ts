...
export function computeTwoPaneWidths(
	...
	const computedLeftWidth = splitPane
		? Math.max(1, Math.floor((innerWidth - 1) * leftPaneRatio))
		: singlePaneWidth;
	const leftWidth = splitPane
		? Math.max(
				1,
				Math.min(computedLeftWidth, leftPaneMaxWidth ?? computedLeftWidth),
			)
		: singlePaneWidth;
