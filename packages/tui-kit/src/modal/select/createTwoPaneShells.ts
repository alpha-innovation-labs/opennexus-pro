 ...
 * @returns Pane shells.
 */
export function createTwoPaneShells(
	options: TwoPaneShellOptions,
): TwoPaneShell[] {
	if (options.showLeftPane && options.showRightPane) {
		const ratio = Math.max(
			...
			Math.min(
				0.9,
				options.leftPaneRatio ?? (options.activePane === "left" ? 0.6 : 0.3),
			),
		);
		return [
			{
				...
				minWidth: Math.min(options.leftPaneMaxWidth ?? 1, options.innerWidth),
			},
			{ id: "right", size: Math.round((1 - ratio) * 100), minWidth: 1 },
		];
	}
	return [
		{ id: options.showLeftPane ? "left" : "right", size: 1, minWidth: 1 },
	];
}
