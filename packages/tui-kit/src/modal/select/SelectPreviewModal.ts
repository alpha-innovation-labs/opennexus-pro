...
export class SelectPreviewModal extends SharedModal {
	...
	constructor(
		...
		this.leftPaneRatio = Math.max(
			0.1,
			Math.min(0.9, options?.leftPaneRatio ?? 0.5),
		);
	...
	setItems(items: AutocompleteItem[]): void {
		this.items = items;
		this.selectList = this.createList(items);
	...
	setHeaderFocusMarkers(visible: boolean): void {
		this.showHeaderFocusMarkers = visible;
	}
	...
	setFooterHintLines(lines: string[]): void {
		this.footerHintLines = lines;
	}

	/** Updates visible pane set. */
	setPaneVisibility(showLeftPane: boolean, showRightPane: boolean): void {
	...
	setModalWidthPolicy(
		minWidth: number,
		maxWidth?: number,
		maxWidthRatio = this.modalMaxWidthRatio,
	): void {
	...
	override handleInput(data: string): void {
		...
		const result = handleTwoPaneRightInput({
			data,
			pendingRightGotoStart: this.pendingRightGotoStart,
			rightLinesLength: this.rightLines.length,
			rightScrollOffset: this.rightScrollOffset,
		});
	...
	override render(width: number): string[] {
		this.syncSharedModalState(width);
	...
	protected jumpSelection(
		predicate: (item: AutocompleteItem) => boolean,
		direction: 1 | -1,
	): void {
	...
	protected focusRightPane(): void {
		if (this.showRightPane) this.activePane = "right";
		this.pendingRightGotoStart = false;
	}
	...
	protected scrollRightToEnd(): void {
		this.rightScrollOffset = Math.max(
			0,
			this.rightLines.length -
				Math.max(1, getTwoPaneBodyHeight(this.fullScreen)),
		);
		this.pendingRightGotoStart = false;
	}

	/** Creates the select-list child. */
	private createList(items: AutocompleteItem[]): PlainSelectList {
	...
	private syncSharedModalState(width: number): void {
		const computedWidth = this.fullScreen
			? width
			: computeModalWidth(width, this.modalMinWidth, this.modalMaxWidthRatio);
		const modalWidth = this.fullScreen
			? width
			: this.modalMaxWidth === undefined
				? computedWidth
				: Math.min(computedWidth, this.modalMaxWidth, width);
		const innerWidth = Math.max(1, modalWidth - 2);
		const footerHintRowCount =
			(this.footerHintLines?.length ?? 0) > 0
				? (this.footerHintLines?.length ?? 0) + 1
				: 0;
		const bodyHeight = Math.max(
			1,
			getTwoPaneBodyHeight(this.fullScreen) - footerHintRowCount,
		);
		const listHeight = this.bottomTitle ? bodyHeight - 2 : bodyHeight;
		this.resizeList(listHeight);
		const shells = createTwoPaneShells({
			activePane: this.activePane,
			innerWidth,
			leftPaneMaxWidth: this.leftPaneMaxWidth,
			leftPaneRatio: this.leftPaneRatio,
			showLeftPane: this.showLeftPane,
			showRightPane: this.showRightPane,
		});
		const widths = computeSharedPaneWidths(
			shells.map((shell) => ({ ...shell, lines: [] })),
			innerWidth,
		);
		const right = createRightPaneLines(
			this.rightLines,
			bodyHeight,
			this.rightScrollOffset,
		);
		this.rightScrollOffset = right.rightScrollOffset;
		const leftLines = this.selectList
			.render(widths[0] ?? innerWidth)
			.slice(0, listHeight);
		while (leftLines.length < listHeight) leftLines.push("");
		this.headerLines = [
			createTwoPaneHeaderLine({
				activePane: this.activePane,
				leftTitle: this.leftTitle,
				rightTitle: this.rightTitle,
				showFocusMarkers: this.showHeaderFocusMarkers,
				leftWidth: widths[0],
				rightWidth: widths.at(-1),
				showLeftPane: this.showLeftPane,
				showRightPane: this.showRightPane,
		...
		const footerSeparator =
			(this.footerHintLines?.length ?? 0) > 0
				? [SHARED_MODAL_FOOTER_BORDER]
				: [];
		this.footerLines = [
			...(this.footerHintLines ?? []),
			...footerSeparator,
			...createTwoPaneFooterLine({
				bottomPrefix: this.bottomPrefix,
		...
		this.panes = [
			...(this.showLeftPane ? [{ ...leftShell, lines: leftLines }] : []),
			...(this.showRightPane ? [{ ...rightShell, lines: right.lines }] : []),
		];
	}
