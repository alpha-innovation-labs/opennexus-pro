 ...
 */
export function handleModalScrollInput(
	state: ModalScrollInputState,
): ModalScrollInputResult {
	...
	if (state.data === Key.ctrl("d") || matchesKey(state.data, Key.ctrl("d")))
		return scrollBy(state, Math.max(1, Math.floor(state.visibleRows / 2)));
	if (state.data === Key.ctrl("u") || matchesKey(state.data, Key.ctrl("u")))
		return scrollBy(state, -Math.max(1, Math.floor(state.visibleRows / 2)));
	return {
		handled: false,
