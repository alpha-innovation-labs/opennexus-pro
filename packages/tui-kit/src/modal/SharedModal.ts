...
import {
	renderModalPaneFillerRows,
	renderModalPanes,
} from "./renderModalPanes";
...
export class SharedModal implements Component {
	...
	setWidthPolicy(
		...
		fullScreen = this.sharedFullScreen,
		fullScreenRows = this.sharedFullScreenRows,
	): void {
	...
	setHidePaneTopBorder(hide: boolean): void {
		this.sharedHidePaneTopBorder = hide;
	}
	...
	render(width: number): string[] {
		const computedWidth = this.sharedFullScreen
			? width
			: computeModalWidth(width, this.minWidth, this.maxWidthRatio);
		const modalWidth = this.sharedFullScreen
			? width
			: this.maxWidth === undefined
				? computedWidth
				: Math.min(computedWidth, this.maxWidth, width);
		const innerWidth = Math.max(1, modalWidth - 2);
		const topBorder =
			this.headerLines.length > 0
				? renderModalBorder(this.theme, "┌", "─", "┐", innerWidth)
				: renderModalBorderWithPaneSeparators(
						this.theme,
						"┌",
						"─",
						"┬",
		...
		if (this.headerLines.length > 0) {
			topRows.push(
				...renderFullWidthRows(this.theme, this.headerLines, innerWidth),
			);
			if (!this.sharedHidePaneTopBorder) {
				topRows.push(
					renderModalPaneTopBorder(this.theme, innerWidth, this.panes),
				);
			}
		}

		const bodyRows = renderModalPanes(this.theme, this.panes, innerWidth);
		const shouldShowBaseHotkeys =
			(this.footerHotkeys !== undefined && this.footerHotkeys.length > 0) ||
			bodyRows.length >
				this.getBodyRowBudget(topRows.length, this.footerLines.length, 1);
		const footerLines = this.getRenderedFooterLines(
			shouldShowBaseHotkeys,
			innerWidth,
		);
		const bottomBorder =
		...
		const bottomRows =
			footerLines.length > 0
				? [
						this.sharedHidePaneTopBorder
							? renderModalBorder(this.theme, "└", "─", "┘", innerWidth)
							: renderModalPaneBottomBorder(this.theme, innerWidth, this.panes),
						...renderFooterRows(this.theme, footerLines, innerWidth),
						bottomBorder,
					]
				: [bottomBorder];
		...
		if (this.sharedFullScreen) {
			...
			bodyRows.push(
				...renderModalPaneFillerRows(
					...
					Math.max(
						0,
						targetRows - topRows.length - bodyRows.length - bottomRows.length,
					),
				),
			);
		}
		...
		const visibleRows = this.getVisibleRows(
			topRows.length + bodyRows.length + bottomRows.length,
		);
		const overflow = renderModalWithScrollableBody(
			this.theme,
			topRows,
			bodyRows,
			bottomRows,
			visibleRows,
			this.overflowScrollOffset,
			this.overflowScrollbar,
		);
		...
		return this.sharedFullScreen
			? overflow.lines
			: overflow.lines.map((line) => centerModalLine(line, width));
	}
	...
	private getFullScreenRows(): number {
		if (typeof this.sharedFullScreenRows === "function")
			return Math.max(1, Math.floor(this.sharedFullScreenRows()));
		if (typeof this.sharedFullScreenRows === "number")
			return Math.max(1, Math.floor(this.sharedFullScreenRows));
	 ...
	 */
	private getBodyRowBudget(
		topRowCount: number,
		footerLineCount: number,
		hotkeyFooterLineCount: number,
	): number {
		const visibleRows = this.getVisibleRows(
			topRowCount + footerLineCount + hotkeyFooterLineCount + 2,
		);
		const footerChromeRows =
			footerLineCount + hotkeyFooterLineCount > 0 ? 2 : 1;
		return Math.max(
			1,
			visibleRows -
				topRowCount -
				footerLineCount -
				hotkeyFooterLineCount -
				footerChromeRows,
		);
	}
	 ...
	 */
	private getRenderedFooterLines(
		showBaseHotkeys: boolean,
		width: number,
	): string[] {
		const segments = createModalHotkeyFooterSegments(
			this.theme,
			this.footerHotkeys ?? [],
			showBaseHotkeys,
		);
		...
		return [
			...wrapModalHotkeyFooterSegments(this.theme, segments, width),
			...this.footerLines,
		];
	}
