...
export function renderSharedModalPaneLines(
	theme: SharedModalTheme,
	pane: SharedModalPane,
	width: number,
): string[] {
	if (pane.contentType !== "markdown") return pane.lines;
	return renderMarkdownPreview({
		markdown: pane.lines.join("\n"),
		theme: {
			style: (token, value) => styleMarkdownPaneToken(theme, token, value),
		},
		width,
	});
}
...
export function renderSharedModalPaneFillerLine(
	theme: SharedModalTheme,
	pane: SharedModalPane,
	width: number,
): string {
	if (pane.contentType !== "markdown") return "";
	return renderMarkdownPreviewLineNumberFiller(
		width,
		pane.lines.join("\n").split("\n").length,
		{ style: (token, value) => styleMarkdownPaneToken(theme, token, value) },
	);
...
function styleMarkdownPaneToken(
	...
	if (
		...
		return theme.fg("syntaxType", value);
	if (
		token === "strikethrough" ||
		token === "horizontalRule" ||
		token === "codeBlock.lineNumber"
	)
		return theme.fg("dim", value);
	if (token === "checkbox.todo") return theme.fg("warning", value);
	if (token === "checkbox.unchecked" || token === "codeBlock.border")
