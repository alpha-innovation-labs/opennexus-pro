...
export function parseInlineSegments(markdown: string): MarkdownInlineSegment[] {
  ...
  for (const match of markdown.matchAll(inlinePattern)) {
    ...
    if (index > cursor) segments.push({ kind: "plain", text: markdown.slice(cursor, index) });
    segments.push(createInlineSegment(match));
    cursor = index + match[0].length;
  }

  if (cursor < markdown.length) segments.push({ kind: "plain", text: markdown.slice(cursor) });
  return segments;
}
