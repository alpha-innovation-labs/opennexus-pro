...
export function renderCodeBlock(
	...
	return [
		renderCodeBlockHeader(block.language, width, theme),
		...block.lines.map((line) => renderCodeBlockLine(line, width, theme)),
		renderCodeBlockBottom(width, theme),
	];
}
...
function renderCodeBlockHeader(
	...
	const icon = styleMarkdownPreviewSegment(
		theme,
		"codeBlock.icon",
		getLanguageIcon(displayLanguage),
	);
...
function renderCodeBlockLine(
	...
	const body = styleMarkdownPreviewSegment(
		theme,
		"codeBlock.background",
		padMarkdownPreviewLine(content, innerWidth),
	);
	return styleMarkdownPreviewSegment(
		theme,
		"codeBlock.border",
...
function renderCodeBlockBottom(
	width: number,
	theme?: MarkdownPreviewTheme,
): string {
	return styleMarkdownPreviewSegment(
