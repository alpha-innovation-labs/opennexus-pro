...
export function renderFrontmatter(
	...
	const label = styleMarkdownPreviewSegment(
		theme,
		"frontmatter.key",
		" frontmatter ",
	);
	...
	rows.push({
		...
		line: `${marker} ${HORIZONTAL_RULE_CHAR.repeat(Math.max(1, Math.floor(ruleWidth / 2)))}${label}${HORIZONTAL_RULE_CHAR.repeat(Math.max(1, Math.ceil(ruleWidth / 2)))}`,
	});
	...
	for (let index = 1; index < lines.length - 1; index += 1) {
		...
		const rendered =
			split === null
				? line
				: `${styleMarkdownPreviewSegment(theme, "frontmatter.key", `${split[1]}: `)}${styleMarkdownPreviewSegment(theme, "frontmatter.value", split[2] ?? "")}`;
		const wrapped = wrapMarkdownPreviewLines(rendered, width);
		rows.push(
			...wrapped.map((part, partIndex) => ({
				sourceLine: partIndex === 0 ? sourceLine : undefined,
				line: part,
			})),
		);
	}
	...
	rows.push({
		sourceLine: startSourceLine + lines.length - 1,
		line: styleMarkdownPreviewSegment(
			theme,
			"frontmatter.border",
			HORIZONTAL_RULE_CHAR.repeat(width),
		),
	});
	return rows;
}
