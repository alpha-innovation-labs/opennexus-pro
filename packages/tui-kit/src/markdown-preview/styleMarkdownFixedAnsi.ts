...
export function styleMarkdownFixedAnsi(token: MarkdownPreviewStyleToken, value: string): string {
  if (token === "lineNumber") return `${lineNumber}${value}${reset}`;
  if (token === "lineNumberSeparator") return `${lineSeparator}${value}${reset}`;
