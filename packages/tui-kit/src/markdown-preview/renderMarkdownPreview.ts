...
import {
	addMarkdownPreviewLineNumbers,
	getMarkdownPreviewNumberedContentWidth,
} from "./addMarkdownPreviewLineNumbers";
import {
	BULLET_MARKERS,
	CHECKBOX_CHECKED,
	CHECKBOX_TODO,
...
export function renderMarkdownPreview(
	...
	const contentWidth = getMarkdownPreviewNumberedContentWidth(
		sourceLines.length,
		width,
	);
	...
	for (let index = 0; index < sourceLines.length; index += 1) {
		...
		if (frontmatter !== undefined) {
			rows.push(
				...renderFrontmatter(frontmatter.lines, 1, contentWidth, options.theme),
			);
		...
		if (codeBlock !== undefined) {
			rows.push(
				...renderCodeBlock(codeBlock.block, contentWidth, options.theme).map(
					(line, offset) => ({ sourceLine: index + 1 + offset, line }),
				),
			);
		...
		rows.push(
			...renderMarkdownPreviewSourceLine(
				sourceLines[index] ?? "",
				index + 1,
				contentWidth,
				options,
	...
	return addMarkdownPreviewLineNumbers(
		rows,
		width,
		sourceLines.length,
		options.theme,
...
function renderMarkdownPreviewSourceLine(
	line: string,
	sourceLine: number,
	width: number,
	options: MarkdownPreviewOptions,
	...
	const listRows = renderMarkdownPreviewListSourceLine(
		line,
		sourceLine,
		width,
		options.theme,
...
function renderMarkdownPreviewListSourceLine(
	line: string,
	sourceLine: number,
	width: number,
	theme: MarkdownPreviewOptions["theme"],
): MarkdownPreviewRow[] | undefined {
	...
	const marker =
		orderedNumber === undefined
			? BULLET_MARKERS[depth % BULLET_MARKERS.length]
			: `${orderedNumber}. `;
	...
	const bodyRows = wrapMarkdownPreviewLines(
		renderParagraph(body, theme),
		bodyWidth,
	);
	return bodyRows.map((bodyRow, index) => ({
		sourceLine: index === 0 ? sourceLine : undefined,
		line: `${index === 0 ? prefix : " ".repeat(prefixWidth)}${bodyRow}`,
	}));
}
...
function renderCheckbox(
	...
	if (state === "-")
		return styleMarkdownPreviewSegment(theme, "checkbox.todo", CHECKBOX_TODO);
	return styleMarkdownPreviewSegment(
		theme,
		"checkbox.unchecked",
		CHECKBOX_UNCHECKED,
...
function renderMarkdownPreviewLine(
	line: string,
	width: number,
	theme: MarkdownPreviewOptions["theme"],
): string | undefined {
	...
	if (/^\s{0,3}([-*_])(?:\s*\1){2,}\s*$/u.test(line))
		return renderHorizontalRule(width, theme);
	const quote = /^(>+)\s?(.*)$/u.exec(line);
	if (quote !== null)
		return renderBlockquote(quote[1]?.length ?? 1, quote[2] ?? "", theme);
	const list = /^(\s*)(?:(\d+)\.\s+|[-*+]\s+)(.*)$/u.exec(line);
	if (list !== null)
		return renderListItem(
			Math.floor((list[1]?.length ?? 0) / 2),
			list[2] === undefined ? undefined : Number(list[2]),
			list[3] ?? "",
			theme,
		);
...
function readFrontmatter(
	lines: string[],
): { lines: string[]; endIndex: number } | undefined {
	if ((lines[0] ?? "").trim() !== "---") return undefined;
	for (let index = 1; index < lines.length; index += 1) {
		if ((lines[index] ?? "").trim() === "---")
			return { lines: lines.slice(0, index + 1), endIndex: index };
	}
	return undefined;
}
...
function readCodeBlock(
	lines: string[],
	startIndex: number,
): { block: MarkdownCodeBlock; endIndex: number } | undefined {
	...
	for (let index = startIndex + 1; index < lines.length; index += 1) {
		if (/^```\s*$/u.test(lines[index] ?? ""))
			return {
				block: { language: opener[1] ?? "", lines: blockLines },
				endIndex: index,
			};
