...
export function addMarkdownPreviewLineNumbers(
	rows: MarkdownPreviewRow[],
	totalWidth: number,
	...
	return rows.map((row) => {
		const numberText =
			row.sourceLine === undefined
				? " ".repeat(digitWidth)
				: String(row.sourceLine).padStart(digitWidth, " ");
		const number = styleMarkdownPreviewSegment(theme, "lineNumber", numberText);
		const separator = styleMarkdownPreviewSegment(
			theme,
			"lineNumberSeparator",
			" │ ",
		);
...
export function renderMarkdownPreviewLineNumberFiller(
	totalWidth: number,
	sourceLineCount: number,
	theme?: MarkdownPreviewTheme,
): string {
	const digitWidth = Math.max(2, String(Math.max(1, sourceLineCount)).length);
	const contentWidth = Math.max(1, totalWidth - digitWidth - 3);
	const number = styleMarkdownPreviewSegment(
		theme,
		"lineNumber",
		" ".repeat(digitWidth),
	);
...
export function getMarkdownPreviewNumberedContentWidth(
	sourceLineCount: number,
	totalWidth: number,
): number {
	const digitWidth = Math.max(2, String(Math.max(1, sourceLineCount)).length);
