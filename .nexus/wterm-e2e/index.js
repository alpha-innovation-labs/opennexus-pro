...
// apps/wterm-e2e/src/socket/createTerminalSocket.ts
function createTerminalSocket(options) {
...
// apps/wterm-e2e/src/ui/extractITermInlineImages.ts
var ITERM_IMAGE_PATTERN = /\u001b\]1337;File=([^:]*):([^\u0007]*)\u0007/g;
function extractITermInlineImages(data) {
  const images = [];
  const text = data.replace(ITERM_IMAGE_PATTERN, (_match, params, base64Data) => {
...
// apps/wterm-e2e/src/ui/findTerminalImageSequenceStart.ts
function findTerminalImageSequenceStart(data) {
  return Math.max(data.lastIndexOf("\x1B]1337;File="), data.lastIndexOf("\x1B_G"), data.endsWith("\x1B") ? data.length - 1 : -1);
}

// apps/wterm-e2e/src/ui/hasCompleteTerminalImageSequence.ts
function hasCompleteTerminalImageSequence(data) {
  if (data.startsWith("\x1B]1337;File=")) return data.includes("\x07");
  if (data.startsWith("\x1B_G")) return data.includes("\x1B\\");
  return !data.endsWith("\x1B");
}

// apps/wterm-e2e/src/ui/createTerminalImageStreamProcessor.ts
var KITTY_CHUNK_PATTERN = /\u001b_G([^;]*);([^\u001b]*)\u001b\\/g;
function createTerminalImageStreamProcessor() {
  let pending = "";
  let kittyParts = [];
  return (data) => {
    ...
    const text = iTerm.text.replace(KITTY_CHUNK_PATTERN, (_match, params, base64Data) => {
...
// apps/wterm-e2e/src/ui/extractTerminalImageUrls.ts
var IMAGE_URL_PATTERN = /https?:\/\/\S+?(?:\.png|\.jpg|\.jpeg|\.webp|\.gif)(?=\s|$)/giu;
function extractTerminalImageUrls(text) {
  return [...text.matchAll(IMAGE_URL_PATTERN)].map((match) => match[0]);
}

// apps/wterm-e2e/src/ui/renderDisconnectedNotice.ts
function renderDisconnectedNotice(term, message) {
  term.write(message);
}

// apps/wterm-e2e/src/ui/createInlineImageUrlElement.ts
function createInlineImageUrlElement(imageUrl) {
...
// apps/wterm-e2e/src/ui/renderTerminalImageUrls.ts
var renderedUrls = /* @__PURE__ */ new Set();
function renderTerminalImageUrls(imageUrls) {
...
// apps/wterm-e2e/src/ui/createInlineImageElement.ts
function createInlineImageElement(image) {
  const element = document.createElement("img");
...
// apps/wterm-e2e/src/ui/renderTerminalInlineImages.ts
function renderTerminalInlineImages(_container, images) {
  for (const image of images) {
    document.body.appendChild(createInlineImageElement(image));
  }
}

// apps/wterm-e2e/src/browser/createTerminalApp.ts
async function createTerminalApp() {
  ...
  const socket = createTerminalSocket({
    ...
    onExit(code) {
      renderDisconnectedNotice(term, `\r
\x1B[33mProcess exited with code ${code}\x1B[0m\r
`);
    },
    onClose() {
...
// apps/wterm-e2e/src/browser/index.ts
async function main() {
