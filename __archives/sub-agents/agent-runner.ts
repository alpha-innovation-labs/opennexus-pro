...
export async function runAgent(
  ...
  const unsubTurns = session.subscribe((event: AgentSessionEvent) => {
    ...
    if (event.type === "tool_execution_start") {
      options.onToolActivity?.({ type: "start", toolCallId: event.toolCallId, toolName: event.toolName, args: event.args });
    }
    if (event.type === "tool_execution_update") {
      const partialResult = event.partialResult as { content?: Array<{ type?: string; text?: string }> } | undefined;
      const outputText = Array.isArray(partialResult?.content)
        ? partialResult.content.filter((block) => block?.type === "text").map((block) => block.text ?? "").join("\n").trim()
        : "";
      options.onToolActivity?.({ type: "update", toolCallId: event.toolCallId, toolName: event.toolName, outputText });
    }
