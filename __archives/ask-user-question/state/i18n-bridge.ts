/**
 * i18n bridge for ask-user-question — single thin import surface so every
 ...
 *
 * - `t(key, fallback)` is `scope("@nexus/ask-user-question")` if the
 *   SDK is installed (live `/languages` updates propagate). If the SDK is
 *   missing (standalone install without i18n), `t` is an identity
...
export const I18N_NAMESPACE = "@nexus/ask-user-question";
...
// i18n), the dynamic import fails, every t(key, fallback) returns the
