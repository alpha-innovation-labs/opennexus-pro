...
export function renderTodoCall(
	...
	} else if (args.action === "create_many" && args.items?.length) {
		text += ` ${theme.fg("dim", `${args.items.length} tasks`)}`;
	} else if (args.action === "delete_many" && args.ids?.length) {
		text += ` ${theme.fg("dim", `${args.ids.length} tasks`)}`;
	} else if (
