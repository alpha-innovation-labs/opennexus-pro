...
- **Agent directory bootstrap**: `packages/nexus-runtime/src/config/ensureAgentDirEnv.ts` sets `PI_CODING_AGENT_DIR` and `NEXUS_CODING_AGENT_DIR` at **module-load time** (top-level code), before any other module executes. It resolves the Nexus default (`~/.local/share/nexus/agent`) unless `NEXUS_CODING_AGENT_DIR` is already set by the user. All downstream path helpers (e.g., `getAgentDir()` from `@earendil-works/pi-coding-agent`) read this env var. No other code should call `ensureAgentDirEnv()` at runtime — the bootstrap runs once when the module is first imported. If you add a new path helper, it must import `ensureAgentDirEnv` (or transitively depend on a module that does) to ensure the env vars are set before the helper runs.
...
- **Herdr mastracode hack**: Herdr does not support "nexus" as an agent type. The npm release wrapper (`scripts/release/npm-package/createReleaseNpmPackage.mjs`) uses `exec -a mastracode` to trick Herdr into tracking the process by the supported name "mastracode". The compiled binary also sets `process.title = APP_NAME` where `APP_NAME = "mastracode"`. Do NOT remove this hack — add a TODO comment if you must touch it. Replace with direct "nexus" once Herdr adds support for it.
