...
function event(overrides: Partial<TuiMouseEvent> = {}): TuiMouseEvent {
	return { type: "wheel", button: "none", x: 2, y: 1, screenX: 2, screenY: 1, width: 40, height: 30,
...
it("caps output at 30 rows, shows a thumb only on overflow, and chains wheel at both ends", () => {
...
it("supports track clicks and thumb dragging without focusing or collapsing", () => {
	...
	expect(viewport.handleMouse(event({ type: "click", button: "left" }))).toMatchObject({ handled: true });
...
it.each(["fixture", "Agent", "SubagentWorkflow", "write", "edit"])("caps %s output and keeps offset through rebuild, resize and collapse", name => {
	...
	expect(rendered.length).toBeLessThanOrEqual(34); // Header and bottom border are outside the 30 output rows.
...
it("scrolls and drags inside a real fullscreen tool while its header remains clickable", () => {
	...
	try {
		...
		expect(lines(row)).toHaveLength(33);
		press(39, bodyY); send(`\x1b[<32;39;${bodyY + 29}M`); release(39, bodyY + 29); tui.renderNow();
		expect(lines(row).join("\n")).toContain("line-099");
