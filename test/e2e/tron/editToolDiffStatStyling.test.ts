...
test("tron edit tool renders git-style added and removed line stats", () => {
  registerToolActivityGroup(["edit-1"]);
  ...
  const component = renderSummary(
    ...
    summarizeArgs("edit", {
      ...
      edits: [{ oldText: "line a\nline b", newText: "line a\nline b\nline c" }],
    }),
    {
  ...
  assert.ok(rawLine, "expected rendered edit tool line");
  assert.match(rawLine, /edit \u001b\[36m\+3\u001b\[0m \u001b\[31m-2\u001b\[0m .*src\/app\/page\.tsx/);

  const plainLine = stripAnsi(rawLine);
  assert.match(plainLine, /edit \+3 -2 .*src\/app\/page\.tsx/);
});
